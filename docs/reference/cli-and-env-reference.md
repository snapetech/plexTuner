...
- `IPTV_TUNERR_STREAM_PROFILES_FILE` — optional JSON object defining **custom profile names** for **`?profile=`** and per-channel overrides. Each value is an object: **`base_profile`** (built-in preset: `default`, `dashfast`, …), optional **`transcode`** (`true`/`false`, default **`true`** for custom names), optional **`output_mux`** (`mpegts` or `fmp4` / `mp4` / `dash` — ffmpeg relay when transcoding), optional **`description`**. See [transcode-profiles](transcode-profiles.md).
...
- `IPTV_TUNERR_METRICS_ENABLE` — when true/`1`/on, register Prometheus mux counters + **`iptv_tunerr_mux_seg_request_duration_seconds`** histogram and expose **`GET /metrics`**.
