...
| `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` | Optional JSON file for file-backed virtual-channel rules used by `/virtual-channels/rules.json` and `/virtual-channels/preview.json`. |
...
- Virtual channels (starter): when `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` is set, Tunerr exposes `/virtual-channels/rules.json` for durable file-backed rules and `/virtual-channels/preview.json` for synthetic channel schedule previews over catalog movies/episodes.
