...
- **Docs:** [plex-livetv-http-tuning](reference/plex-livetv-http-tuning.md) links **`X-IptvTunerr-Native-Mux`** to **[hls-mux-toolkit](reference/hls-mux-toolkit.md)**; [hybrid-hdhr-iptv](how-to/hybrid-hdhr-iptv.md) See also → mux toolkit + troubleshooting. **`k8s/README.md`** verify snippet includes **`/healthz`**.
...
- **Deck auth + persisted memory**: the dedicated deck now opens on its own login page with cookie-backed sessions, while still accepting direct HTTP Basic auth for scripts (`admin` / `admin` by default via **`IPTV_TUNERR_WEBUI_USER`** / **`IPTV_TUNERR_WEBUI_PASS`**), and can persist shared telemetry/history across restarts with **`IPTV_TUNERR_WEBUI_STATE_FILE`**. Runtime snapshot + deck UI now show whether auth is still on the default password and whether memory is durable or only process-local.
...
- **Editable deck controls**: the dedicated web UI now exposes **`/deck/settings.json`** for safe updates to deck username/password and default refresh cadence, persists those settings when **`IPTV_TUNERR_WEBUI_STATE_FILE`** is configured, and surfaces the live deck-security posture directly in the Settings lane.
