...
- **DASH / HLS mux polish:** optional **`IPTV_TUNERR_HLS_MUX_DASH_EXPAND_SEGMENT_TEMPLATE`** (+ **`_MAX_SEGMENTS`**) expands uniform **`SegmentTemplate`** to **`SegmentList`** for VoD-style MPDs; HLS playlists normalize non-standard **`#EXTINF:...,BYTERANGE=...`** into separate **`#EXTINF`** + **`#EXT-X-BYTERANGE`** lines (bis-style tags); **`/debug/runtime.json`** includes **`tuner.hls_mux_dash_expand_segment_template`** / **`tuner.hls_mux_dash_expand_max_segments`**.
