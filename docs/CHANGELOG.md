...
- **Guide-input sandboxing:** local XMLTV / alias file refs now resolve only within the current working directory or explicit **`IPTV_TUNERR_GUIDE_INPUT_ROOTS`** entries, remote XMLTV / alias fetches are limited to hosts already declared in provider/XMLTV/HDHR config (plus optional **`IPTV_TUNERR_GUIDE_INPUT_ALLOWED_HOSTS`** entries), and guide parsing now reads through a single validated load path instead of reopening separate local-file sinks.
