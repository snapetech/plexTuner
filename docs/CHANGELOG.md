...
- **Deck auth + persisted memory**: the dedicated deck now requires HTTP Basic auth (`admin` / `admin` by default via **`IPTV_TUNERR_WEBUI_USER`** / **`IPTV_TUNERR_WEBUI_PASS`**) and can persist shared telemetry/history across restarts with **`IPTV_TUNERR_WEBUI_STATE_FILE`**. Runtime snapshot + deck UI now show whether auth is still on the default password and whether memory is durable or only process-local.
