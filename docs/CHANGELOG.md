...
### CI
- **Linux workflows use the paired local runner pool:** Linux jobs now target the repo-specific `iptvtunerr` self-hosted label so either `kspld0` or `kspls0` can take amd64 or arm64-cross build work.
