...
- **Plex lineup harvest bridge (`LH-004` / `LH-005` starter)**: saved harvest reports can now live in `IPTV_TUNERR_PLEX_LINEUP_HARVEST_FILE`, surface through `/programming/harvest.json`, and flow into `/programming/preview.json` so Programming Manager and the deck can show harvested candidate lineups instead of treating harvest results as one-shot CLI output only.
- **Virtual channels starter (`PAR-006` slice)**: `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` now enables file-backed virtual-channel rules plus `/virtual-channels/rules.json` and `/virtual-channels/preview.json`, giving operators a durable schedule-preview substrate for synthetic channels built from catalog movies/episodes.
