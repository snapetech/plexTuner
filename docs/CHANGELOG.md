...
- **Non-HTTP HLS mux targets:** direct **`?mux=hls&seg=`** requests with unsupported target schemes (for example **`skd://...`**) now return **`400 Bad Request`** with a clear error string instead of a generic `502`.
