...
- **Docker image can be republished on demand:** the Docker workflow accepts a `workflow_dispatch` tag input to (re)publish a release image without re-pushing the git tag.
