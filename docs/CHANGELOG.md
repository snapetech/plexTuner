...
- **INT-010 / active remediation (host quarantine):** optional **`IPTV_TUNERR_PROVIDER_AUTOTUNE_HOST_QUARANTINE`** — when autotune is on, **`walkStreamUpstreams`** drops quarantined hosts (failure threshold + cooldown) if backup URLs remain. **`/provider/profile.json`**: **`auto_host_quarantine`**, **`quarantined_hosts`**, **`penalized_hosts[].quarantined_until`**, **`remediation_hints`**. **`/debug/runtime.json`** **`tuner.provider_autotune_host_quarantine_*`**. See **cli-and-env** / **`.env.example`**.
