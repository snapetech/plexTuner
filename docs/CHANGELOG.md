...
### Plex Live TV proxy
- **Plex HLS relay stability guidance:** docs now call out the remux-first posture for Plex Live TV feeds where PMS already transcodes remote playback, including when to disable forced Go relay, leave the stdin normalizer off, and keep ffmpeg on the original CDN hostname.
