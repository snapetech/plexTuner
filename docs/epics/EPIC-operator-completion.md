...
| CMP-010 | HDHR+provider merge hardening | Merge/precedence logic for hybrid lineup and provider catalogs has deterministic conflict policy under edge cases (dup names, same ID, same source family). | `internal/hdhomerun/*`, `internal/catalog/*`, `internal/tuner/server.go` | merge fixtures + integration tests + host/manual smoke |
