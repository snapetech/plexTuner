...
- Next recommended slices (still open — validated 2026-03-19):
  ...
  - **Autopilot**: provider-level / multi-host **policy** memory — **`IPTV_TUNERR_AUTOPILOT_GLOBAL_PREFERRED_HOSTS`** plus **`IPTV_TUNERR_AUTOPILOT_HOST_POLICY_FILE`** (preferred + blocked hosts) are now shipped; remaining depth is dynamic/UI policy and stronger automatic strip/cap decisions beyond guarded block lists
