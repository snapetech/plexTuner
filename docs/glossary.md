...
| **transcode** | Converting a video stream's codec/container with ffmpeg before passing it to Plex. `IPTV_TUNERR_STREAM_TRANSCODE=auto` uses ffprobe per tune and transcodes only if Plex can't play it natively (e.g. HEVC, VP9). `auto_cached` + `IPTV_TUNERR_TRANSCODE_OVERRIDES_FILE` is remux-first with no per-tune probe. |
...
| **websafe** | A streaming mode that applies browser-compatible codec settings (MP3 audio) and startup race hardening (bootstrap TS, PAT+PMT keepalive) for Plex Web/browser clients. The ffmpeg **startup gate** treats **H.264 IDR** and **HEVC IRAP** (NAL types 16–21) as a decodable video keyframe when **`IPTV_TUNERR_WEBSAFE_REQUIRE_GOOD_START`** is on. |
| **client class** | Tunerr’s coarse Plex client bucket for adaptation and Autopilot: **`web`**, **`internal`** (PMS/Lavf fetcher), **`native`** (typical TV/phone apps), or **`unknown`** when the session cannot be resolved. See [plex-client-compatibility-matrix](reference/plex-client-compatibility-matrix.md). |
