...
# Run IPTV Tunerr without Kubernetes

Run IPTV Tunerr on a single host (bare metal, VM, or container) with no cluster. Three ways: **binary**, **Docker**, or **systemd**.

See also: [README](../../README.md) (quick start), [k8s/README.md](../../k8s/README.md) (cluster deploy), [iptvtunerr-troubleshooting](../runbooks/iptvtunerr-troubleshooting.md).
...
```bash
# Copy binary and config to a dedicated directory (e.g. /opt/iptvtunerr)
