...
| Stream freezes after playing for a while, especially sports HLS | Provider playlist refresh hit a per-account/session cap, or stale shared provider-account leases make Tunerr think account capacity is still occupied | Compare `/debug/active-streams.json` with `/provider/profile.json`. If `account_leases` is greater than active streams, stale lease files are likely involved. Keep `IPTV_TUNERR_PROVIDER_ACCOUNT_SHARED_LEASE_TTL=2m`; active streams heartbeat their leases, while dead sessions clear quickly. Grep logs for `playlist refresh concurrency-limited`, `status=509`, `hls relay stalled after progress`, and repeated same-channel `/stream/<id>` requests |
