...
| **Programming Manager foundations** | Server-backed curation primitives now exist: `/programming/categories.json` builds grouped source/category inventory from the raw post-intelligence lineup, `/programming/recipe.json` persists a durable lineup recipe, and `/programming/preview.json` shows the curated lineup after the recipe is applied. This is the backend foundation for the upcoming category-first channel-builder UI. |
