...
| **Catch-up capsule publishing** | `/guide/capsules.json` and `catchup-capsules` still expose the preview/feed layer, and `catchup-publish` now turns those capsules into real `.strm + .nfo` lane libraries plus `publish-manifest.json`, with optional Plex/Emby/Jellyfin library create+refresh. `IPTV_TUNERR_CATCHUP_GUIDE_POLICY=healthy|strict` can suppress weak placeholder-only channels from preview/publish output. Emby and Jellyfin were live-validated in-cluster; Jellyfin uses its current `GET /Library/VirtualFolders` + query-param `POST /Library/VirtualFolders` API shape. |
