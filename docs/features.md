...
| **Physical HDHomeRun client (spike)** | `hdhr-scan` discovers real tuners on the LAN (UDP, global + optional **`IPTV_TUNERR_HDHR_DISCOVER_BROADCASTS`**) or fetches `discover.json` / `lineup.json` via HTTP; catalog merge is **not** automatic — see [ADR 0002](adr/0002-hdhr-hardware-iptv-merge.md). |
...
| **Provider behavior profile foundation** | `/provider/profile.json` exposes learned provider/runtime quirks (tuner cap, concurrency/CF signals, penalized hosts, mux counters, last **HLS/DASH** mux outcomes) plus **`intelligence.autopilot`** — a trimmed Autopilot hot-list and decision count for one-fetch operator dashboards ([LTV epic](epics/EPIC-live-tv-intelligence.md)). |
