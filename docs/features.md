...
| **Virtual channels (starter+)** | `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` enables a file-backed virtual-channel rules model for owned media/catalog sources. `/virtual-channels/rules.json` stores the durable ruleset, `/virtual-channels/preview.json` renders a schedule preview over movies/episodes, `/virtual-channels/schedule.json` exposes a rolling synthetic schedule horizon, `/virtual-channels/live.m3u` publishes the enabled synthetic channel rows, `/virtual-channels/stream/<id>.mp4` proxies the current scheduled asset, `/virtual-channels/channel-detail.json` gives focused rule/current-slot/schedule detail, and `/virtual-channels/guide.xml` now emits a synthetic XMLTV view so the feature is moving from “preview/playback experiment” toward a real publishable TV-like surface. |
