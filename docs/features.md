...
| **Active stream view** | `/debug/active-streams.json` exposes currently in-flight stream sessions with request IDs, channels, start times, durations, and current tuner occupancy. |
| **Xtream-compatible live output (starter)** | Optional read-only downstream Xtream surface: `player_api.php?action=get_live_streams|get_live_categories` plus `/live/<user>/<pass>/<channel>.ts`, backed by the curated lineup and existing stream gateway. |
