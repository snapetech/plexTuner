...
| **Autopilot decision memory foundation** | Optional JSON-backed memory file remembers winning playback decisions by `dna_id + client_class`, so repeated Plex Web/native/internal requests can reuse a known-good transcode/profile choice. `autopilot-report` and `/autopilot/report.json` expose the hottest remembered channels, and hot-start rules can use those hits to tighten startup behavior for high-traffic channels. |
