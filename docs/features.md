...
| **Virtual channels (starter)** | `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` enables a first file-backed virtual-channel rules model for owned media/catalog sources. `/virtual-channels/rules.json` stores the durable ruleset, and `/virtual-channels/preview.json` renders a schedule preview over movies/episodes so operators can design synthetic channels before real playback/publishing is layered on. |
