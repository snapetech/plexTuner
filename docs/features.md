...
| **`/healthz` / `/readyz`** | Catalog readiness for operators: **`/readyz`** returns JSON **`ready`** / **`not_ready`** (**503** until **`UpdateChannels`** has live rows); **`/healthz`** returns **`ok`** / **`loading`** plus **`source_ready`**, **`channels`**, **`last_refresh`**. Example **`host/`** readiness probes use **`/readyz`**; **`/discover.json`** is often a better **liveness** target during long cold starts. See [runbook §8](runbooks/iptvtunerr-troubleshooting.md#8-tuner-endpoints-sanity-check). |
...
| **Dedicated web UI (`:48879` / `0xBEEF`)** | Separate operator dashboard on `serve`/`run` with a single-origin proxy over tuner JSON/debug endpoints plus a runtime-settings snapshot (`/debug/runtime.json`). Default: `http://127.0.0.1:48879/`; opens on a dedicated login page with cookie-backed deck sessions, while direct HTTP Basic auth still works for scripts; if `IPTV_TUNERR_WEBUI_PASS` is unset, Tunerr generates a one-time startup password instead of defaulting to `admin/admin`; localhost-only unless `IPTV_TUNERR_WEBUI_ALLOW_LAN=1`; optional `IPTV_TUNERR_WEBUI_STATE_FILE` persists server-derived operator activity and non-secret deck preferences across web UI restarts. The deck now also has explicit keyboard/focus affordances: skip-to-content, stronger focus-visible states, labeled controls, and accessible modal semantics instead of relying only on raw browser prompts. |
| **Deck actions / workflows / memory** | The deck exposes safe operator actions (`/ops/actions/*`), workflow/playbook endpoints (`/ops/workflows/*`), a read-only deck telemetry surface (`/deck/telemetry.json`), server-derived operator activity (`/deck/activity.json`), and refresh-preference controls (`/deck/settings.json`) with grouped endpoint indexing, trends, and a runtime-backed settings lane. The same Settings lane now surfaces the live shared-replay window and virtual-channel live-stall watchdog, can apply new `shared_relay_replay_bytes` / `virtual_channel_recovery_live_stall_sec` values for future sessions through the localhost-only operator action path, and persists those runtime knobs across restarts when `IPTV_TUNERR_WEBUI_STATE_FILE` is configured. **Provider profile** cards summarize flat **`/provider/profile.json`** (tuner limits, penalties, **host quarantine** skips + quarantined-host counts when relevant, mux/autopilot breadcrumbs, **Autopilot consensus** when computed) and surface **`remediation_hints`** on Overview/Routing (incidents + decision board when warn-level hints exist). The Routing lane now also surfaces **`/debug/shared-relays.json`** so operators can see active shared live sessions, shared-upstream family, and current subscriber counts instead of treating replay-backed session reuse as invisible runtime behavior. Virtual-station field editing now uses the deck modal/editor surface for branding and recovery values instead of `window.prompt` popups, so those mutations stay inside the same keyboard-navigable operator flow. |
| **Diagnostics workflow + evidence intake** | `/ops/workflows/diagnostics.json` turns recent stream attempts into a concrete good-vs-bad capture checklist with suggested channel IDs, the latest `.diag/` run families, and summarized verdict/findings from the newest `channel-diff`, `stream-compare`, `multi-stream`, or evidence-bundle artifacts. `/ops/actions/evidence-intake-start` scaffolds a fresh `.diag/evidence/<case-id>/` bundle directly from the operator plane, `/ops/actions/channel-diff-run` and `/ops/actions/stream-compare-run` can now launch bounded local captures directly, and the deck surfaces the whole diagnostics lane instead of leaving the workflow buried in shell scripts. |
...
| **Xtream-compatible output (expanded starter)** | Optional read-only downstream Xtream surface: `player_api.php` now serves `get_live_streams`, `get_live_categories`, `get_vod_categories`, `get_vod_streams`, `get_series_categories`, `get_series`, `get_series_info`, `get_short_epg`, and `get_simple_data_table`; `get.php` now publishes a user-scoped M3U playlist; and `xmltv.php` now publishes a user-scoped XMLTV guide built from Tunerr's existing guide and virtual-schedule pipeline. `/live/<user>/<pass>/<channel>.ts`, `/movie/<user>/<pass>/<id>.mp4`, and `/series/<user>/<pass>/<episode>.mp4` proxy through Tunerr-owned URLs instead of exposing raw upstreams directly. Short-EPG responses are backed by Tunerr's existing guide and virtual-schedule pipeline for both real live channels and virtual channels, so downstream Xtream consumers can ask for compact now/next listings without a separate guide export. Virtual channels now also appear as downstream Xtream live streams and play back through the same `/live/...` surface, so the owned-media schedule path is no longer stranded outside the client-facing output. A file-backed multi-user entitlement layer (`IPTV_TUNERR_XTREAM_USERS_FILE`, `/entitlements.json`) can now scope live/VOD/series access per downstream user instead of treating the Xtream output as one global view. |
...
| **Programming Manager** | Server-backed lineup curation primitives now exist and are operable from the dedicated control deck. `/programming/categories.json` builds grouped source/category inventory and supports bulk category include/exclude, `/programming/browse.json` gives one category’s channel rows with cached guide-health/next-hour EPG status in one request, `/programming/channels.json` handles exact channel include/exclude overrides, `/programming/channel-detail.json` gives focused channel metadata plus a 3-hour upcoming-programme preview and exact-match alternative sources, `/programming/order.json` persists manual server-side order changes, `/programming/backups.json` reports exact-match sibling groups and now supports durable preferred-primary selection, `/programming/recipe.json` stores the durable recipe, and `/programming/preview.json` shows the curated lineup plus taxonomy bucket counts after the recipe is applied. Programming rows now also carry derived feed descriptors (`region | category | feedtype/fps-style tags`) so the deck can show Strong8K-style context without rewriting canonical names, `order_mode: "recommended"` produces a deterministic Local/News/Sports/etc. ordering on the server, `collapse_exact_backups: true` can collapse only strong same-channel siblings into one visible row with merged fallback streams, `/virtual-channels/schedule.json` now gives the deck a real synthetic schedule horizon, and the web UI Programming lane includes in-place live HLS preview, category browse with “Real Guide Only” / “Only Not In Lineup” quick filters, focused detail, alternative-source context, bounded `stream-compare` and exact-backup `channel-diff` launchers, plus one-click backup preference from the same lane instead of only cards and JSON drill-down. |
| **Plex lineup harvest** | `iptv-tunerr plex-lineup-harvest` productizes the old wizard-oracle flow: it can probe several tuner lineup variants against Plex, poll channel-map results, capture discovered lineup titles/URLs plus harvested lineup rows, and emit a structured report plus deduped lineup summary for later Programming Manager or wizard-lane decisions. The saved report file (`IPTV_TUNERR_PLEX_LINEUP_HARVEST_FILE`) is now visible through `/programming/harvest.json`, embedded into `/programming/preview.json`, surfaced in the deck’s Programming lane, importable through `/programming/harvest-import.json` to seed a real saved recipe from a chosen harvested lineup, and the import report now explains how rows matched back onto the current catalog (`tvg_id`, exact name, guide number, or local-broadcast stem fallback). `/programming/harvest-assist.json` now ranks those harvested lineups as real local-market recipe assists, and the deck can preview/apply those assists directly instead of leaving them as text-only guidance. |
| **Virtual channels (starter+)** | `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` enables a file-backed virtual-channel rules model for owned media/catalog sources. `/virtual-channels/rules.json` stores the durable ruleset, `/virtual-channels/preview.json` renders a schedule preview over movies/episodes, `/virtual-channels/schedule.json` exposes a rolling synthetic schedule horizon and now also accepts basic authoring mutations (`append_entry`, `replace_entries`, `append_movies`, `append_episodes`, `remove_entries`), `/virtual-channels/report.json` gives a condensed per-station operator view with recovery summary fields, `/virtual-channels/live.m3u` publishes the enabled synthetic channel rows, `/virtual-channels/stream/<id>.mp4` proxies the current scheduled asset, `/virtual-channels/channel-detail.json` gives focused rule/current-slot/schedule detail and now also accepts station-metadata updates, `/virtual-channels/recovery-report.json` exposes recent filler/recovery events, `/virtual-channels/branded-stream/<id>.ts` provides the first branded playback surface, and `/virtual-channels/guide.xml` emits a synthetic XMLTV view so the feature is moving from “preview/playback experiment” toward a real publishable TV-like surface. Virtual-channel rules can also carry station-oriented metadata such as `description`, `branding.logo_url`, bug/banner hints, recovery/filler policy metadata, and a per-channel `branding.stream_mode` publish override; the deck Programming lane can now mutate `stream_mode`, `logo_url`, `bug_image_url`, `bug_text`, `bug_position`, `banner_text`, `theme_color`, `recovery.mode`, and `recovery.black_screen_seconds` directly, logo export is already wired through M3U/XMLTV, branded playback can now burn text plus a corner image overlay, and runtime filler/recovery execution now covers startup probes, repeated rolling in-session media-content probes, repeated live stall/error cutovers across the ordered fallback chain, and explicit exhaustion events when the chain runs out instead of stopping at startup-only checks. |
| **Shared live session reuse** | Same-channel duplicate consumers can now reuse one upstream/live output instead of always starting another provider walk. The native `hls_go` path exposes `/debug/shared-relays.json` with current shared sessions and subscriber counts, the live FFmpeg HLS remux/transcode path now lets later viewers attach to the existing producer when the output shape matches (including named-profile `fMP4` output), and profile-selected ffmpeg packaged HLS (`output_mux: "hls"`) reuses the same packaged session for later viewers when the requested output profile matches. Shared sessions now keep a bounded replay buffer too, so late subscribers get recent startup bytes instead of joining completely cold. |
| **Migration workflows (media servers)** | Tunerr now has built-in builder / convert / diff / rollout-diff / audit / apply lanes for Plex-to-Emby/Jellyfin overlap work. `live-tv-bundle-*` handles tuner and guide registration, `library-migration-*` handles shared library definitions and attached catch-up publish manifests, and `migration-rollout-audit` plus the deck Migration workflow summarize readiness, conflicts, index state, library presence, population hints, scan state, and parity hints across one or both destination targets. |
| **Identity migration workflows** | `plex-user-bundle-build`, `identity-migration-convert`, `identity-migration-diff`, `identity-migration-apply`, and the rollout/audit variants now cover destination local-account bootstrap plus the first safe additive policy sync inferred from Plex share state (`AllowTuners`, `AllowSync`, `AllLibraries`, and shared-user remote access). The deck Identity workflow surfaces the same readiness state, missing users, policy-update users, activation-pending users, and manual follow-up hints from the running appliance. |
| **OIDC / IdP migration workflows** | `identity-migration-oidc-plan` emits a provider-agnostic OIDC user/group plan from the Plex-user bundle, and live Keycloak/Authentik diff/apply commands now reconcile users, Tunerr-owned groups, group membership, and Tunerr migration metadata. The deck OIDC workflow can audit and apply that saved plan, capture recent successful and failed IdP pushes, filter history to `all / success / failed`, and show per-target outcome rows for partial Keycloak/Authentik runs directly in the workflow modal. |
| **Legacy `/ui/` shell** | Embedded lightweight pages remain on the tuner port for compatibility and quick read-only access: home (`/ui/`) links to health and JSON reports; **`/ui/guide/`** shows a read-only preview of the merged cached guide; localhost-only unless `IPTV_TUNERR_UI_ALLOW_LAN=1`. The legacy pages now explicitly present themselves as a compatibility shell and point operators back to the dedicated Control Deck as the primary operator surface. |
...
| **Guide health report** | `guide-health` and `/guide/health.json` inspect the actual merged guide output and classify channels by real programme coverage, sparse coverage, placeholder-only fallback, or no guide rows at all. |
...
| **Lineup recipes** | Intelligence-driven lineup shaping with `IPTV_TUNERR_LINEUP_RECIPE=high_confidence|balanced|guide_first|resilient|sports_now|sports_na|kids_safe|locals_first`. |
| **Guide-quality lineup policy** | Optional runtime policy (`IPTV_TUNERR_GUIDE_POLICY=healthy|strict`) can suppress channels that only produce sparse rows, placeholder rows, or no real programme coverage once guide-health cache is available. |
| **Guide-policy decision surface** | `/guide/policy.json?policy=healthy|strict` turns cached guide-health into an explicit keep/drop report with counts for healthy, sparse, placeholder-only, no-programme, and missing-`TVGID` decisions, so lineup and catch-up policy behavior is inspectable instead of implicit. |
| **Registration recipes** | `IPTV_TUNERR_REGISTER_RECIPE=healthy|balanced|high_confidence|guide_first|resilient|sports_now|sports_na|kids_safe|locals_first` lets Plex/Emby/Jellyfin registration reuse channel-intelligence scoring or the same intent-oriented lineup recipes instead of blindly registering catalog order. |
...
| **host cutover examples** | Example JSON/manifests and URI cutover mapping included. |
...
| **External helper (Python)** | `built-in Plex session reaper` remains available for lab/host workflows. |
...
| **Numbered env slots** | `IPTV_TUNERR_FREE_SOURCE_2=https://example.invalid/free-source-2.m3u` adds another public source directly from env without rewriting the `FREE_SOURCES` CSV. |
...
| **Live TV provider label and entitlement proxy** | `iptv-tunerr plex-label-proxy` can rewrite `/media/providers` labels per DVR/provider and, when explicitly run with `-elevate-live-tv`, elevate only Plex Live TV requests to the PMS owner token so non-Home users keep their own library sessions while Live TV borrows owner tuner entitlement. See [plex-live-tv-entitlement-proxy](runbooks/plex-live-tv-entitlement-proxy.md). |
| **VPN access patterns** | Tailscale, WireGuard, OpenVPN, Gluetun, NAT-PMP/static-forward, and fail-closed routing patterns for Tunerr provider egress and Plex Live TV proxy access are documented in [vpn-access-patterns](reference/vpn-access-patterns.md). |
