...
| **Diagnostics workflow + evidence intake** | `/ops/workflows/diagnostics.json` turns recent stream attempts into a concrete good-vs-bad capture checklist with suggested channel IDs, the latest `.diag/` run families, and summarized verdict/findings from the newest `channel-diff`, `stream-compare`, `multi-stream`, or evidence-bundle artifacts. `/ops/actions/evidence-intake-start` scaffolds a fresh `.diag/evidence/<case-id>/` bundle directly from the operator plane, and the deck now surfaces both as a first-class diagnostics capture lane instead of leaving the workflow buried in shell scripts. |
