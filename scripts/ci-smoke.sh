...
grep -q '<channel id="smoke.one">' <(curl -sS "http://127.0.0.1:$port_full/xmltv.php?username=demo&password=secret") || fail "xtream xmltv export missing entitled channel"
...
grep -q '<channel id="smoke.one">' <<<"$limited_xmltv" || fail "limited xtream xmltv missing allowed channel"
! grep -q '<channel id="smoke.two">' <<<"$limited_xmltv" || fail "limited xtream xmltv leaked denied channel"
