...
- **LP progress (2026-03-19):** **LP-001** MVP = **`hdhomerun.DiscoverLAN`** + **`hdhr-scan`** + HTTP **`discover.json`/`lineup.json`/`guide.xml`** clients on **`internal/httpclient`**. **LP-010** partial = hardware profile aliases in **`gateway_profiles.go`**. **LP-004–LP-006** largely covered by **`internal/webui`** + tuner JSON. **Next multi-PR block:** **LP-002** (merge ADR + lineup import from hardware), **LP-007–LP-009** (SQLite + incremental + cleanup), **LP-011** (ffmpeg profile execution matrix), **LP-012** doc sweep. **Park:** Postgres multi-writer EPG, always-on recorder daemon, provider incremental **xmltv.php** without contract — see [opportunities.md](opportunities.md).
