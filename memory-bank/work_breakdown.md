...
- **ACC progress (2026-03-21):** `ACC-001`, `ACC-002`, and the first visible `ACC-003` slice are in: the gateway now tracks active provider-account leases, prefers lower-load accounts during URL ordering, can reject locally when every distinct account for a channel is already at `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT`, and exposes current account lease state plus the configured cap on provider/runtime surfaces. **New depth landed:** per-account concurrency-limit learning from real upstream `423` / `458` / `509`-style signals, surfaced as `account_learned_limits` on `/provider/profile.json`. Remaining depth is persistence/decay policy, not basic learning.
