...
- **Progress (2026-03-21):** first visible `PAR-006` slice shipped too: `IPTV_TUNERR_VIRTUAL_CHANNELS_FILE` now enables file-backed virtual-channel rules, `/virtual-channels/rules.json` stores or mutates them, `/virtual-channels/preview.json` renders schedule previews over catalog movies/episodes, and the starter now publishes enabled synthetic rows via `/virtual-channels/live.m3u` plus a current-slot asset proxy at `/virtual-channels/stream/<id>.mp4`.
