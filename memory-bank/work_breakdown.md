...
- **LP progress (2026-03-20):** **LP-001**–**LP-003**, **LP-007**–**LP-011** shipped per [EPIC-lineup-parity](../docs/epics/EPIC-lineup-parity.md) **Current status**; **LP-012** / hybrid how-to ongoing. **LTV overlay:** **`/provider/profile.json`** → **`intelligence.autopilot`** (hot Autopilot sample) for deck/scripts; epics’ **Implementation status** text deduped with **Current status**. **Next product depth:** **LP-010/011** ffmpeg **disk** HLS packaging (optional); **LP-001** IPv6/multicast polish. **Park:** Postgres, always-on recorder beyond daemon MVP, provider incremental **xmltv.php** — [opportunities.md](opportunities.md).
