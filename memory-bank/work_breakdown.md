...
- **LP progress (2026-03-19):** **LP-001** MVP = **`hdhomerun.DiscoverLAN`** + **`hdhr-scan`** + HTTP **`discover.json`/`lineup.json`/`guide.xml`** clients on **`internal/httpclient`**. **LP-010 / LP-011** now include **`IPTV_TUNERR_STREAM_PROFILES_FILE`** (named profile matrix: **`base_profile`**, **`transcode`**, **`output_mux`**) plus mux-aware ffmpeg relay selection when no explicit **`?mux=`** is supplied; native mux responses advertise **`X-IptvTunerr-Native-Mux`** (**`hls`**/**`dash`**) for operators. HDHR-style aliases remain in **`gateway_profiles.go`**. **LP-004–LP-006** largely covered by **`internal/webui`** + tuner JSON. **Next multi-PR block:** **LP-002** (merge ADR + lineup import from hardware), **LP-007–LP-009** (SQLite + incremental + cleanup), **LP-012** doc sweep. Example **k8s** readiness probes may use **`GET /readyz`** or **`GET /healthz`** (503 until channels loaded). **Park:** Postgres multi-writer EPG, always-on recorder daemon, provider incremental **xmltv.php** without contract — see [opportunities.md](opportunities.md).
