...
- **The host-local LG TV lane still appears to require full video transcode for reliability, but browser playback can use the lighter audio-normalization path:** updated 2026-03-22 after moving the split-profile runtime from `plexsafehq` to the new higher-quality `plexsafemax`. Current mitigation is:
  ...
  - unresolved/resolved PMS internal fetcher lane => `plexsafemax`
  - both behind the adaptation `websafe` branch, while global `stream_transcode` remains `off`
  - Impact: the TV picture can be improved by tuning the TV-safe profile, but a fully non-transcoding TV path still has not been proven reliable on this PMS/LG combination.
  - Open experiment: `plexsafeaac` now exists in code as the same high-quality TV-safe video lane with AAC audio instead of MP3, but it has not yet been validated live. Treat it as experimental until a clean LG/PMS A/B proves it does not reintroduce the older playback/session failures.

- **Plex Web and native TV clients currently need different fallback profiles on the host-local `:5005` helper; one shared WebSafe profile is not stable for both lanes:** confirmed on 2026-03-22 after iterative A/B runs against the LG TV (`192.168.50.225`) and Plex Web/Desktop (`192.168.50.148`). Evidence:
  - `copyvideomp3` is good enough for browser playback because it keeps source video and only normalizes audio/subtitle baggage.
  - The LG TV still stalled/spun on Plex's post-session segment consumption under that same safe-audio path, even though Tunerr itself was serving stream bytes successfully.
  - Switching the ambiguous/internal PMS fetcher lane back to `plexsafehq` made TV playback work again.
  - Root cause: the helper often sees only `Lavf/...` / `PlexMediaServer/...` on `/stream/...`, so unresolved TV and browser requests can collapse into the same "internal fetcher" shape unless adaptation explicitly splits them.
  - Current mitigation: use `IPTV_TUNERR_PLEX_WEB_CLIENT_PROFILE=copyvideomp3` and `IPTV_TUNERR_PLEX_INTERNAL_FETCHER_PROFILE=plexsafehq` while keeping `IPTV_TUNERR_FORCE_WEBSAFE_PROFILE` as the general fallback.
  - Impact: quality/perf tuning for host-local Plex Live TV should be evaluated per client lane, not as a single global "websafe profile" switch.

- **PMS can serve an entirely blank XMLTV provider tab even while Tunerr `lineup.json` and `guide.xml` are populated, if the DVR's enabled channel set drifts away from the current channelmap generation:** Confirmed on 2026-03-22 for host-local DVR `723` (`tv.plex.providers.epg.xmltv:723`). Evidence:
  - Tunerr still exposed `479` lineup rows and a non-empty `guide.xml`.
  - But the exact TV-facing PMS endpoint `GET /tv.plex.providers.epg.xmltv:723/lineups/dvr/channels` returned `<MediaContainer size="0">`, and `.../hubs/discover` was also empty.
  - Direct `GET /livetv/epg/channelmap?...device=device://tv.plex.grabbers.hdhomerun/hdhrlocal&lineup=...#hdhrlocal` returned `479` rows, of which `380` were fully valid mappings and `99` were incomplete.
...
- **The recreated host-local Plex DVR now tunes real video channels, but radio-only channels still fail later in PMS recorder startup and the enabled mapping count appears to collapse after activation:** Updated on 2026-03-22 after exposing Tunerr's `/auto/` route on the local `http://127.0.0.1:5005` helper and replaying manual PMS tune requests on DVR `723`. Evidence:
  ...
  - After mounting `/auto/`, PMS tunes enabled video channel `43784` (`FR| CANAL+ FOOT FHD`) end-to-end: it fetches `http://127.0.0.1:5005/stream/1002837`, returns `/livetv/sessions/.../index.m3u8`, and PMS serves `.ts` segment bytes from that session.
  - Enabled radio channel `43401` still fails differently: PMS reaches the tuner and receives bytes, but its first-stage recorder/transcoder aborts that session with `Output file does not contain any stream` / `Recorder: Unable to create transcode session or session failed to start`.
  - Root cause of the temporary `475`→`75` collapse: `internal/plex.ActivateChannelsAPI` batched mappings in groups of 100, but each request only sent the current batch in `channelsEnabled`; Plex treats that field as a full replacement, so the last 75-row batch won. After reapplying activation with the full enabled set on every batch, DVR `723` returned to `475` enabled mappings.
  - Remaining guide issue: provider XMLTV still fails (`HTTP 403` / `513`) on the current provider hosts. An opt-in fallback via `IPTV_TUNERR_PROVIDER_SHORT_EPG_FALLBACK=true` now synthesizes real programme blocks from upstream `player_api.php?action=get_short_epg` for a subset of channels, but only 31 of the 479 lineup channels currently have real schedule rows. Impact: Plex clients may still show a mixed guide where some channels get timeline blocks and many remain flat placeholder rows.
  - Smart-TV-specific follow-up: the browse attempt from `192.168.50.225` on `45955` failed earlier and differently from the later backend recorder failures. PMS logged `Got exception from request handler: Invalid argument` during `GET /video/:/transcode/universal/decision?...path=/livetv/sessions/...` before the player session really started, while backend/manual tune runs on the same channel can run for 40s+ and only fail later when upstream playlist refresh hits provider `509` concurrency limits. The live-session XML Plex produces for both `45955` and `43784` still reports `audioChannelLayout="0 channels"` and `samplingRate="0"` even though direct `ffprobe` on `http://127.0.0.1:5005/stream/1002837` eventually sees valid AAC stereo 48 kHz. Impact: there is likely an early startup/probing defect in the stream shape Plex sees first, not just a dead tuner or permanently bad media stream.
  - Current host-side workaround under test: run the local `:5005` tuner with `IPTV_TUNERR_FORCE_WEBSAFE=true`, `IPTV_TUNERR_FORCE_WEBSAFE_PROFILE=plexsafehq`, and `IPTV_TUNERR_FFMPEG_NO_DNS_RESOLVE=true`. The second flag matters for this provider/CDN mix: with FFmpeg DNS rewrite enabled, `relayHLSWithFFmpeg` rewrote `line.dambora.xyz` to a numeric Cloudflare IP and failed in the startup gate with `Invalid data found when processing input`. The profile flag matters too: the older default `plexsafe` workaround forced low bitrate and could produce visibly poor quality or odd aspect handling on TV clients. `plexsafehq` keeps the compatibility-important MP3 audio path but adds `setsar=1` and raises the bitrate/mux ceilings substantially. Impact: if smart-TV playback regresses on the host-local DVR, re-check both `IPTV_TUNERR_FFMPEG_NO_DNS_RESOLVE=true` and the forced profile before chasing Plex DB/API theories again.
  - Updated 2026-03-22: the host-local helper no longer has to stay on blanket `FORCE_WEBSAFE`. `requestAdaptation` now exposes `IPTV_TUNERR_PLEX_UNKNOWN_CLIENT_POLICY`, `IPTV_TUNERR_PLEX_INTERNAL_FETCHER_POLICY`, and `IPTV_TUNERR_PLEX_RESOLVE_ERROR_POLICY`, so the local helper can run remux-first again even when PMS forwards weak/no client identity. Verified live on `:5005` with all three set to `direct`: fresh stream probe on `15578` logged `plex-hints none` followed by `hls-mode transcode=false mode="off"` and `ffmpeg-remux profile=default`.
  - Updated 2026-03-22 again: the ambiguous no-hints PMS/Lavf lane now has a deterministic inference step, not just sticky fallback after a failure. When the helper receives an internal fetcher request with no forwarded Plex session/client IDs, it can query `/status/sessions` and inherit the single plausible non-internal client (or the sole web client when no native competitor exists). Impact: Plex Web should no longer need a first failed attempt before getting the `copyvideomp3` safe-audio branch, while genuinely native clients can still remain on direct/remux if they are the only active non-internal session. Limitation: if multiple plausible non-internal clients are active at once, the helper still falls back to the old ambiguous handling/sticky path rather than guessing.
  - Remaining limitation: this is only a **first-attempt** fix, not a perfect automatic fallback. The earlier smart-TV remux failures happened later inside PMS (`/video/:/transcode/universal/decision` 400/500) after Tunerr had already produced a healthy stream, so current sticky/autopilot fallback cannot always detect and react to that class from the original `/stream/...` request alone.
  - Impact: the old dead-device theory is fixed for the main local DVR, and playback is confirmed on some video channels, but client-visible success still depends on a usable video channel plus either working short-EPG fallback coverage or a richer external/provider XMLTV source; radio/audio-only handling, smart-TV startup compatibility, and full provider EPG access still need work.

- **Non-Home Plex shares are currently hard-clamped away from Live TV tuner access at plex.tv share state, even when local PMS DVR/provider rows are present:** Confirmed on 2026-03-22 by direct local DB inspection plus live plex.tv API experiments. Local `com.plexapp.plugins.library.db` and per-DVR XMLTV SQLite hold tuner/provider/lineup state, but they do not appear to contain the decisive non-Home share entitlement. `GET https://plex.tv/api/users`, `GET https://plex.tv/api/servers/<processed-machine-id>/shared_servers`, and `GET https://plex.tv/api/v2/shared_servers/<share-id>` expose per-user `home` and `allowTuners` state. Recreating an existing non-Home share via `POST https://plex.tv/api/v2/shared_servers` while requesting `settings.allowTuners=1` succeeds with `201`, but the resulting share is observed with `allowTuners="0"`. Row-level `POST https://plex.tv/api/v2/shared_servers/<share-id>` is also writable, but current tests show it mutates library membership/share shape without bypassing the tuner clamp. Meanwhile `api/v2/home/users` is read-only from this account (`allow: OPTIONS, GET, HEAD`), so an obvious promote-to-Home write path has not yet been found. Impact: host-side DB injection alone is not currently a proven workaround for ordinary non-Home shared users on smart-TV clients. Current repo tooling for this investigation: `iptv-tunerr plex-db-inspect`, `plex-log-inspect`, `plex-api-inspect`, `plex-api-request`, and `plex-share-force-test`.
...
- **External XMLTV on `/guide.xml` can still block Plex metadata flows on cache misses:** `internal/tuner/xmltv.go` caches successful remapped XMLTV responses (default TTL 10m), but cache misses still synchronously fetch/remap the external XMLTV feed. In live k3s testing on 2026-02-24, `iptvtunerr-websafe` `guide.xml` took ~45s with `IPTV_TUNERR_XMLTV_URL` enabled, causing Plex DVR metadata/channel APIs to hang or time out.
  ...
  - Follow-up finding (same day): with a direct IptvTunerr catalog filtered to EPG-linked channels and deduped by `tvg-id` (91 channels), the same external XMLTV remap path served `guide.xml` in ~1.0–1.3s with real guide data. **Update:** merged-guide **TTL cache** (`XMLTV.cachedXML`, default 10m) makes steady-state `/guide.xml` cheap; **cache misses** (cold start or TTL expiry) can still block for large catalogs + slow upstreams.
...
- **Plex Live TV can show injected DVRs as `Unavailable` even for the owner when the registered tuner URIs point at cluster-only names PMS cannot resolve:** Reconfirmed on 2026-03-22 during smart-TV browse capture.
  - Symptom: the TV guide shows the injected DVR rows, but opening them reports `Unavailable`; Plex-hosted channels still play.
  - File-log evidence: PMS repeatedly logs `DVR:Device: Error refreshing existing device ... marking as dead` followed by `Device ... died` for `device://tv.plex.grabbers.hdhomerun/*` rows whose URIs are `http://plextuner-*.plex.svc:5004`.
  - Host evidence: from the PMS host, those `.plex.svc` names fail DNS resolution, while candidate `.plex.home` names currently answer `404 page not found` on `/discover.json`.
  - Impact: this is a separate failure mode from Plex Home / `allowTuners` gating. Even the owner cannot tune those DVRs until the registered URIs resolve and serve a valid HDHomeRun HTTP surface from the PMS host.
  - What works: use `iptv-tunerr plex-device-audit` to prove reachability, then `iptv-tunerr plex-dvr-cutover` with a TSV map only after the replacement URIs return working `/discover.json` and `/lineup_status.json`.

- **Plex `GET /livetv/epg/channelmap` can return malformed `ChannelMapping` rows without `channelKey` / `lineupIdentifier`, and feeding those back into `PUT /media/grabbers/devices/<id>/channelmap` causes duplicate-mapping activation failures:** Reconfirmed on 2026-03-22 while re-registering a host-local tuner.
  - Symptom: channel activation fails partway through with Plex reporting a duplicate channel mapping even though the visible `channelmap` payload mostly looks unique.
