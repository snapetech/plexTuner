...
- **VODFS FUSE mounts become invisible inside the Plex pod after a pod restart if `mountPropagation: HostToContainer` is not set on the hostPath volumes:** Observed on 2026-02-28.
  ...
  - All 11 VODFS lane processes are now managed by systemd on kspls0 (`plextuner-vodfs-*.service`, enabled, restart-on-failure). See `k3s/plex/VODFS-SYSTEMD.md` for unit table and recovery procedure.
  - Recovery if mounts are still empty after pod restart: `sudo systemctl status 'plextuner-vodfs-*'` on kspls0 to confirm all units running, then `mount | grep fuse` to confirm 11 FUSE mounts. If a unit is down, `sudo systemctl restart plextuner-vodfs-<lane>`. Pod no longer needs to be restarted for mount visibility once `HostToContainer` propagation is set.
