...
| **`internal/tuner/`** | HDHR endpoints, stream gateway, XMLTV/guide pipeline, Autopilot, Ghost Hunter, provider profile, catch-up publishing. `server.go` now holds the main runtime/bootstrap, lineup shaping, and remaining shared server helpers; `server_status_reports.go` owns health/readiness and read-only report/status endpoints; `server_operator_workflows.go` owns operator workflow/action surfaces and harvest-request plumbing; `server_programming.go` owns the programming-manager/report/import surfaces; `server_virtual_channels.go` owns the operator-facing virtual-channel management/reporting surfaces; `server_virtual_channel_streams.go` owns the virtual playback/recovery transport stack; `server_diagnostics_recordings.go` owns the diagnostics harness and recorder/debug endpoint tail. |
| **`internal/webui/`** | Dedicated deck on port `48879` (`0xBEEF` by default); reverse-proxies tuner JSON/debug endpoints under `/api/*`, presents a readiness-first setup/health path, hides the raw endpoint atlas plus the advanced `ops` workflow lane by default, and now also demotes diagnostics/migration/recovery workflow actions back to Settings unless the user explicitly enables advanced surfaces, keeping only guide and stream repair as first-class workflow lanes in simple mode and routing the last diagnostics/harvest direct actions back through Settings unless advanced mode is enabled and labeling advanced summary cards more explicitly in simple mode. `webui.go` now focuses on listener/proxy/index/state wiring, `webui_auth.go` owns auth/session/security helpers, `webui_setup.go` owns the first-run setup report endpoint, and `webui_migration.go` owns migration/OIDC workflow/report handlers. |
...
- **`internal/webui/webui.go`** — Dedicated dashboard listener + `/api/*` reverse proxy + shared deck state/persistence wiring; main page is embedded from `internal/webui/index.html`. Supporting splits: **`webui_auth.go`** (auth/session/security), **`webui_setup.go`** (first-run setup report), **`webui_migration.go`** (migration/OIDC workflow/report handlers).
- **`internal/tuner/server.go` + `internal/tuner/operator_ui.go`** — Main tuner runtime, lineup shaping, and the remaining shared server helpers plus the legacy compatibility UI shell. Supporting splits: **`server_status_reports.go`** (health/report/status JSON surfaces), **`server_operator_workflows.go`** (safe operator actions/workflows plus harvest-request plumbing), **`server_programming.go`** (programming-manager/report/import handlers and helpers), **`server_virtual_channels.go`** (virtual-channel rules/preview/schedule/report/guide handlers and mutations), **`server_virtual_channel_streams.go`** (virtual-channel slate/stream/recovery/branding transport), and **`server_diagnostics_recordings.go`** (diagnostics harness helpers, recorder endpoints, HLS demo, mux decode action, and `device.xml`), all guarded by the localhost/LAN UI policy.
