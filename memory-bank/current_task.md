**Latest (2026-04-21):** **Preparing `v0.1.53` for the sports freeze fix:** user approved releasing the stale provider-account lease and empty shared-relay fix rather than treating it as local config only. Release scope includes runtime changes from `657411b`, CI action cleanup from `3248ac3`, updated env reference/default docs, troubleshooting guidance for comparing `/debug/active-streams.json` and `/provider/profile.json`, and changelog section `v0.1.53`.

**Latest (2026-04-21):** **Sports freeze traced to stale provider-account shared leases plus empty transcode shared relays:** user reported a freeze on `CA| SPORTSNET WEST FHD` (`904206`). Live logs showed the stream stalled after provider `509` playlist refreshes, and `/provider/profile.json` showed one active stream but `line.dambora.xyz` shared lease `in_use=3`. The extra lease files were stale (`01:32`, `03:10`) because shared provider-account lease TTL was `6h`. Fix now live on both cluster deployments as image `localhost/iptvtunerr:cluster-51fb168-freezefix`: shared lease TTL is `2m`, expired lease files are removed during profile snapshots, and the Go HLS transcode path no longer creates empty shared relay sessions that cannot fan out replay bytes. Live validation: sports settled to `78` channels after `drop_placeholders`; `904206` ran a 90s `Lavf/60.16.100` sample with `HTTP 200` and `246,397,688` bytes; no `509`/stall logs appeared during the sample, and account leases were empty after client disconnect. Verification: `go test -count=1 ./internal/tuner` and `./scripts/verify`.

**Latest (2026-04-21):** **Fixing the post-`v0.1.51` release workflow Node 20 warning:** the release succeeded, but GitHub Actions warned that `softprops/action-gh-release@v2` runs on deprecated Node 20. Upstream `softprops/action-gh-release` latest is `v3.0.0` and its action metadata uses `node24`, so both `.github/workflows/release.yml` and `.github/workflows/tester-bundles.yml` are being moved to `softprops/action-gh-release@v3`; `v0.1.52` will be cut as the verification tag for the warning fix.

**Latest (2026-04-21):** **Sports probe regression fixed live after validation showed real black feeds and bad EPG exposure:** user was right to challenge the release state. Live sports on `v0.1.50` exposed `99` rows with `18` `placeholder_only` guide rows, and logs showed `BEIN SPORTS HD` / `PRIME| TENNIS CHANNEL` tuning HLS playlists whose first media segment was `/video/black.ts`. Root cause: `ProbeStream` accepted HLS as soon as it saw `#EXTM3U`, before scanning media URI lines for `black.ts`; visual probe only checked event-like candidates, so generic sports slates survived. Fix now live on `deployment/iptvtunerr-sports` image `localhost/iptvtunerr:cluster-b29eb54-sportsfix3`: HLS probes scan the playlist before accepting, gateway rejects placeholder media lines at tune time, sports guide policy is `drop_placeholders`, and reapply is guarded so the filtered guide-health snapshot does not re-expand placeholder rows. Stable live result after watchdog cycle: `/readyz` `75`, `/discover.json` `TunerCount=87`, guide health `0` `placeholder_only`, `75` real-programme rows, `21` no-programme rows in the source-health report, Plex watchdog `enabled=75/75`, and old black IDs `/stream/1836256`, `/stream/1920568`, `/stream/45611` return `404`.

**Latest (2026-04-21):** **Publishing sports lineup probing as release `v0.1.50`:** `v0.1.49` was pushed but the GitHub Release workflow failed during Windows artifact build because shared provider-account lease locking used Unix-only `syscall.Flock`. The runtime sports probing work was already live-tested successfully in cluster; the release follow-up moves shared-lease locking behind platform helpers so Unix keeps real `flock` behavior and Windows release binaries compile. Proceeding with `v0.1.50` so the public release notes include the sports probing feature set and the release-build fix.

**Latest (2026-04-21):** **Sports visual black probing is live and measured:** added optional ffmpeg black-frame probing after the fast lineup probe. The cluster sports DVR now runs fast feed probing with concurrency `4` / max duration `60s`, then visual event probing with sample `4s`, timeout `9s`, concurrency `1`, max feeds `12`, max duration `45s`. Measurement on the live sports catalog: no probe had effectively `0s` app-side lineup delay and `130` channels; cold fast probe at old settings hit `2m` and kept `92/130`; cold fast+visual old settings took `2m16.8s` and kept `104/130` with visual dropping `1`; tuned fast+visual took `1m13.7s` in one run and final manifest-aligned live run took `1m42.6s`, keeping `78/130` and dropping `4` additional visual-black event feeds/channels. Live state: image `localhost/iptvtunerr:cluster-a6bd4e9-sports-visualprobe`, `/readyz` reports `78`, `/discover.json` reports `TunerCount=88`, and active streams are `0/88`. Tradeoff: useful but cold-start expensive; consider lowering visual max feeds or persisting `/var/cache/iptvtunerr` if startup delay is too high.

**Latest (2026-04-21):** **Adding sports lineup probing before Plex sees the guide:** user confirmed they want the sports DVR to probe candidate feeds before inserting channels into `lineup.json`/`guide.xml`, especially to avoid black/placeholder feeds. Scope is a low-load runtime lineup probe enabled on the sports deployment only: probe final curated feeds, prune failing stream URLs from each channel, drop channels with no valid feeds, reject empty direct streams and obvious `black.ts`/`blank.ts` placeholders, cache results briefly, and let `TunerCount=auto` derive from the surviving feed URLs.

**Latest (2026-04-21):** **Sports tuner/account capacity now derives from valid exposed feeds instead of a static number:** user correctly rejected arbitrary `1`/`2` limits because the sports DVR is meant for multi-user, multi-stream access. Code now supports `IPTV_TUNERR_TUNER_COUNT=auto` and `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT=auto`. Auto tuner count is computed from the unique non-empty upstream stream URLs in the exposed lineup; provider account pool auto uses the same lineup feed capacity unless a specific upstream identity learns a lower provider limit. Live sports manifest now sets both to `auto`, image `localhost/iptvtunerr:cluster-a6bd4e9-sports-autocap`. Live proof: `/readyz` reports `130` channels; `/lineup.json` exposes 130 HDHR URLs; the underlying sports channels contain `145` unique provider feed URLs because some channels have backups; `/discover.json` reports `TunerCount:145`; `/provider/profile.json` reports `configured_tuner_limit=145`, `effective_tuner_limit=145`, and `account_pool_limit=145`; `/debug/active-streams.json` reports `tuner_limit=145`.

**Latest (2026-04-21):** **Sports concurrency was restored after user pushback; keep multi-stream goal separate from per-feed health filtering:** a brief attempt to clamp sports to `1` stream was based on live `509` overlap from the provider, but that conflicts with the product goal of multiple users/streams and should not be the default answer. The sports deployment is back to `IPTV_TUNERR_TUNER_COUNT=2` and `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT=2`; live `/provider/profile.json` reports `configured_tuner_limit=2`, `effective_tuner_limit=2`, and `account_pool_limit=2`. The stale/slate lineup fixes remain: current sports image `localhost/iptvtunerr:cluster-a6bd4e9-sports-staleblack`, `/readyz` reports `130` channels, Peacock rows are currently `0`, and the known bad patterns (`SPORTSNET PPV`, `NHL TEAM`, `NFHS`, `ENDED`) are `0`. Future provider `509` overlaps should be handled as upstream health/failover/credential capacity work, not by silently reducing the sports DVR to one stream.

**Latest (2026-04-21):** **Sports black-screen reports were generic slate rows, not only Peacock:** after the near-live Peacock filter, fresh Plex logs showed black-screen attempts tuning `CA| SPORTSNET PPV` and `NHL TEAM| EDMONTON OILERS`; both returned large byte counts (`111,688,732` and `173,401,800`) but were likely provider slate/black video. `sports_na` now excludes generic non-event `PPV`, `TEAM|` rows, `NFHS` PPV rows, and `ENDED` event rows while preserving NBA Pass PPV and ABC/NBC/ESPN carriage. Live rollout image: `localhost/iptvtunerr:cluster-a6bd4e9-sports-slatefilter2`; sports `/readyz` reports `133` channels, Plex activated all `133`, and the lineup scan shows `0` `SPORTSNET PPV` / `NHL TEAM` / `NFHS` / `ENDED` rows, with `12` ABC, `19` NBC/NBC Sports, `7` ESPN, and `1` Peacock row still present.

**Latest (2026-04-21):** **Peacock client spinner required removing far-future event rows from the tunable lineup, not just returning cleaner 502s:** after the zero-byte fix, the user still saw a spinner because Plex retried future Peacock event rows (`/stream/1910864`, then `/stream/1910846`) and kept polling tuner status after receiving clean failures. Those rows were scheduled far outside the current playable window and the provider returned `200 video/mp2t` with zero bytes. `sports_na` now excludes event rows with provider timestamps more than `IPTV_TUNERR_SPORTS_EVENT_FUTURE_WINDOW` ahead of `timeNow()` (default `4h`) while keeping active/near-live event rows. Live rollout image: `localhost/iptvtunerr:cluster-a6bd4e9-sports-nearlive`; sports `/readyz` reports `158` channels, Plex API registration activated all `158`, far-future Peacock IDs `/stream/1910846` and `/stream/1910864` now return `404`, and remaining Peacock `/stream/1910874` streamed `22,948,276` bytes over a 15s sample.

**Latest (2026-04-21):** **Peacock spinning is narrowed to provider-empty future event rows, and the sports pod now fails those cleanly instead of committing an empty stream:** the user-reported Peacock spinner matched `/stream/1910846` (`US (Peacock 031) | Game 3: Pistons vs. Magic (2026-04-25 12:00:00)`). Before the follow-up fix, the provider returned `HTTP 200` / `video/mp2t` but closed with zero bytes after ~21s; Tunerr had already sent `200` to Plex, so Plex spun waiting for media. `relaySuccessfulRawUpstream` now pre-reads the first chunk for Peacock raw `.ts` bypass streams and treats a zero-byte body as `raw_empty_body` before writing headers, allowing Plex to receive a clear `502` / tune failure. Live sports rollout image: `localhost/iptvtunerr:cluster-a6bd4e9-peacock-emptyfail`. Validation: `/readyz` still reports `165` sports channels; `/stream/1910846` now returns `502` with `all_upstreams_failed` and `EOF`; `/stream/1910874` (`US (Peacock 003) | Game 2: Raptors vs. Cavaliers`) still streamed `26,490,916` bytes over a 20s sample.

**Latest (2026-04-20):** **Sports NBA/carriage expansion is live with Peacock fixed instead of dropped:** live cluster inspection showed `sports_na` is not Canadian-only, but it originally exposed only 106 sports rows from a 51k-channel catalog and only three NBA TV-style rows. The raw catalog had current NBA event feeds (TSN+, Peacock, NBA Pass PPV), but those rows lacked `.ca/.us` TVG IDs and were pruned by the NA shape/guide path. The recipe now recognizes NA NBA/WNBA event rows while still excluding `NO EVENT STREAMING` placeholders and non-NA NBA Pass buckets; the sports manifest keeps `IPTV_TUNERR_GUIDE_POLICY=off` so event rows can reach Plex. Peacock was restored and fixed by normalizing Peacock event stream URLs from broken/hanging `.m3u8` endpoints to `.ts`, bypassing ffmpeg for Peacock MPEG-TS, and omitting extra Basic auth on Peacock `.ts` requests because this provider/CDN returns zero-byte bodies when both path credentials and Basic auth are present. `sports_na` also now includes capped ABC/NBC broadcast carriage rows (12 each, guide-linked only) so games carried on ABC/NBC are visible in the sports DVR without flooding Plex with hundreds of affiliates. Live final rollout: `localhost/iptvtunerr:cluster-a6bd4e9-sports-carriage-capped`; sports `/readyz` reports `165` channels with `12` ABC, `7` ESPN, `19` NBC/NBC Sports, `10` Peacock, and `7` Prime/Amazon-style rows. Peacock `/stream/1910864` returned `14,472,616` bytes through Tunerr; `/stream/1910860` is currently provider-empty even when fetched directly, so that row remains dependent on upstream event activation.

**Latest (2026-04-20):** **Stream-drop mitigation lane:** I updated both cluster Tunerr deployments to increase concurrent tuning capacity from 1→2 (`IPTV_TUNERR_TUNER_COUNT`, `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT`) and restored bounded concurrency retries (`UPSTREAM/HLS retry limit=2`, plus `*_RETRY_BACKOFF_MS=1000`) so short provider lock contention has recovery time before Plex receives a hard 503. Next step: apply manifests, roll both Deployments, and re-check `/provider/profile.json` + `/debug/stream-attempts.json` for any remaining `provider_accounts_in_use` rejections.

**Latest (2026-04-20):** **Sports now also needs healthy guide pruning at deploy time, not just the short-EPG credential fix:** after shipping `b759ef3`, the cluster reached `dev-b759ef3` and the runtime fix worked as intended for provider-identity-aware short-EPG fallback, but live sports guide health still settled at `106` channels with `80` real-programme rows and `26` `placeholder_only` rows. That remaining gap is no longer “short EPG used the wrong credentials”; it is that the sports deployment still exposed placeholder-only rows because it lacked `IPTV_TUNERR_GUIDE_POLICY=healthy`, unlike primary. The cluster manifest now adds that same healthy guide policy to `deploy/cluster/plex/iptvtunerr-sports-deployment.yaml` so sports should only expose guide-healthy rows to Plex after the next deploy. Verification is green again: YAML parse for the manifest and full `./scripts/verify`. Immediate next step: push the sports manifest update, let the cluster auto-deploy, and confirm sports lineup/guide health drops from `106` to the real-programme subset instead of serving placeholder rows.

**Latest (2026-04-20):** **Sports guide repair now targets the remaining fixable short-EPG gap instead of Plex registration:** live validation on `dev-baff8a0` showed the cluster and Plex DVRs were on the newest push and mapped correctly (`803` sports `88/88`, `806` primary `406/403` with the known Plex `dead` noise), but sports guide health still had only `41` channels with real provider programmes and `23` `placeholder_only` rows. The fixable code issue was in `internal/tuner/epg_pipeline.go`: provider XMLTV already merged multiple provider identities, but short-EPG fallback still tried alternate provider base URLs using only the primary provider credentials. That meant channels whose usable short-EPG source lived behind a different credential set could never gap-fill. The runtime now walks full provider identities for short-EPG fallback, preferring identities that match the channel-derived base URL and using each identity’s own `user/pass`, with regression coverage in `TestFetchProviderShortEPGFallback_UsesMatchingProviderIdentityCredentials`. Verification is green: `go test ./internal/tuner` and full `./scripts/verify`. Immediate next step: push this fix to `origin/main`, let the cluster auto-deploy, and re-check sports `/guide/health.json` to see how many placeholder-only rows remain after the live refresh.

**Latest (2026-04-19):** **Cluster auto-deploy now avoids doc-only churn and no longer forces a restart on every successful `main` push:** the disruptive part of the old lane was not just that deploys were serialized; it was that `.github/workflows/deploy-cluster.yml` ran as a separate `workflow_run`, always rebuilt `localhost/iptvtunerr:cluster`, and always `rollout restart`ed both single-replica Tunerr deployments. That meant even memory-bank-only pushes bounced live pods. The lane now runs as a `Deploy Cluster` job inside `CI` after `verify` and `smoke`, filters for cluster-relevant paths only, builds per-SHA images (`localhost/iptvtunerr:cluster-<sha>`), applies the manifests, and updates deployment images directly instead of forcing a restart. Manual deploys still exist through `.github/workflows/deploy-cluster.yml`, but that workflow is now `workflow_dispatch` only and uses the same `scripts/deploy-cluster.sh` helper as CI. Resume verification is now complete: `bash -n scripts/deploy-cluster.sh`, YAML parse for both workflows, focused `go test ./cmd/iptv-tunerr ./internal/tuner`, and full `./scripts/verify` all passed. While resuming, the `CI` deploy job also picked up the same cluster concurrency guard as the manual workflow, and the runtime EPG fallback logic was tightened so programme-less fallback repair no longer overrides a valid provider-first match.

**Latest (2026-04-19):** **The remaining provider-drop question is now narrowed correctly:** the shared provider-account lease rollout is live and proven, so the primary/sports overlap path no longer double-books upstream. During live validation, primary held `/stream/177396` while sports requesting `/stream/904205` was rejected locally with `503`, `X-HDHomeRun-Error: 805`, and `reject provider-accounts-in-use limit=1`; that means the cross-pod account-coordination fix is working as designed. The remaining risk is narrower: shared HLS relay reuse is still process-local, so it only collapses duplicate pulls when overlapping clients land on the same pod and same relayable request shape. There was no fresh `509`, `shared-hls-relay`, or account-limit churn in the follow-up 20-minute log slice after rollout, so the unresolved gap is now observability plus possible cross-pod relay reuse, not “we need separate credentials” as a first answer. GitHub `CI`, `Docker`, `Gitleaks`, and `CodeQL` passed for `fb109bd`, and the `Deploy Cluster` workflow is running from `main`.

**Latest (2026-04-19):** **Cross-pod provider-account enforcement now works live, not just per process:** the remaining playback-drop risk after the lineup/watchdog fixes was that `iptvtunerr` and `iptvtunerr-sports` still shared the same upstream account while each pod only enforced its own local `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT=1`. Code now supports `IPTV_TUNERR_PROVIDER_ACCOUNT_SHARED_LEASE_DIR`, `..._TTL`, and `..._OWNER`, using a node-shared filesystem lease with heartbeat renewal and stale-lease cleanup. Both cluster manifests now mount `/var/lib/iptvtunerr/provider-account-leases` from `kspld0`, so primary and sports see one combined account pool. Live proof after rebuild/import/rollout: while primary held `/stream/177396`, a sports request for `/stream/904205` returned `503` with `X-HDHomeRun-Error: 805` and body `All provider accounts in use`, and sports logs showed `reject provider-accounts-in-use limit=1` without touching upstream. This closes the cross-pod double-booking path even though the upstream account is still shared.

**Latest (2026-04-19):** **Primary lineup collapse and DVR self-rebuild churn are fixed live, and the GitHub-to-cluster deploy lane is ready:** the real cluster regression was not a fresh stream-relay bug; it was the primary lineup/guide path collapsing itself from `479 -> 285 -> 93 -> 82` because deferred guide-policy reapplication kept filtering the already-filtered lineup. Code now keeps a stable pre-policy source lineup for guide-health decisions, applies guide policy once at the end of curation, and rebuilds deferred policy from `RawChannels` only when guide-health is actually ready. Live result after rebuild/import/rollout: primary starts at `479`, applies one healthy-guide drop to `406`, and stays there; Plex DVR `806` remains `406/406` enabled. The watchdog also no longer tears the DVR down just because Plex marks the device `dead`; it now leaves healthy registrations in place when enabled mappings still match the exposed lineup. Live proof: watchdog cycles now log `status="dead" ... but enabled=406/406; leaving registration in place`, and a 4-minute in-pod `Lavf/60.16.100` sample on `/stream/177396` held until the client timeout with HTTP `200` and `493,532,900` bytes. Residual live risk is still upstream provider playlist `509` during long playback; the sample logged one `playlist concurrency learned provider-account limit=1 status=509` near the end without dropping the stream, so a shared cross-pod provider lease or separate sports credentials remains the durable fix. The main-branch deploy workflow remains `.github/workflows/deploy-cluster.yml`, backed by the repo-scoped runner `kspld0-iptvtunerr` and the custom manifests under `deploy/cluster/plex/`.

**Latest (2026-04-19):** **Stream disconnect triage found and fixed the live watchdog churn, and deploy automation is now wired:** a 6-minute in-pod `Lavf/60.16.100` stream sample on `/stream/177396` completed with HTTP `200`, `921,147,360` bytes, and final status `ok`; the app did not reproduce a tuner-side disconnect in that sample. The concrete live bug was the primary Plex DVR watchdog reactivating every minute because it still compared Plex enabled mappings against the initial pre-guide-policy 479-channel registration set while the live exposed lineup had been filtered to ~402-406 guide-healthy channels. That generated repeated `GET /guide.xml`, channelmap fetches, and `ActivateChannelsAPI` calls during playback. Code now makes the watchdog read the current tuner `/lineup.json` count and only falls back to the original registration list if the live lineup cannot be fetched. The fix is rebuilt/imported and rolled to both cluster Tunerr deployments. A repo-scoped self-hosted GitHub Actions runner `kspld0-iptvtunerr` is online with labels `self-hosted, iptvtunerr-deploy, kspld0`, and `.github/workflows/deploy-cluster.yml` plus `deploy/cluster/plex/*` now define the main-branch cluster deploy path: after CI passes on `main`, build `localhost/iptvtunerr:cluster` on `kspld0`, import to k3s, apply the custom primary/sports manifests, restart both deployments, and smoke `/readyz`.

**Latest (2026-04-19):** **Missing EPG and buffering fixes are deployed to the cluster:** live guide-health had reported all primary channels as `good`, but direct `/guide.xml` inspection showed many one-row channel-name placeholder blocks (for example `CA| CTV VANCOUVER HD` from `2026-04-19T00:39:28Z` to `2026-04-27T00:39:28Z`). Code now parses all XMLTV display names, prefers non-numeric names for placeholder detection, reports true one-row real coverage as `sparse`, and makes guide policy drop sparse/placeholder rows. `IPTV_TUNERR_GUIDE_POLICY=healthy` is enabled on the primary DVR only, and the runtime now reapplies deferred guide policy once guide health becomes ready. Live result after rebuild/import/rollout: primary dropped `74/479` placeholder-only rows and now serves `405` channels with `0` sparse, `0` placeholder-only, and `0` no-programme rows; sports remains `106`. Buffering root cause from logs: primary and sports pods share the same provider account, and a primary CTV stream hit provider playlist `509` / account limit while each pod only enforces its own local `max 1`; provider-3 credentials in the secret did not authenticate. Mitigation deployed: both deployments set stream and playlist retry limits to `0` so provider-limit failures fail/retune faster instead of sitting through retry backoff. Durable follow-up remains a shared cross-pod provider-account lease or a valid separate sports credential.

**Latest (2026-04-18):** **Live client-drop mitigation is applied and rolled out:** the root cause was a provider concurrency mismatch, especially on sports. Cluster health was OK (`iptvtunerr`, `iptvtunerr-sports`, and `plex-standby` Ready; Plex has only DVRs `752` primary and `755` sports, both alive), but `iptvtunerr-sports` had been using two tuners while the upstream returned playlist `509` under overlap and Tunerr logged `playlist concurrency learned provider-account limit=1`. Concrete failing sample: overlapping Plex `Lavf/60.16.100` pulls for `CA| TSN 4 HD` (`/stream/177347`) reached `inuse=2/2`; the second path hit two `playlist refresh concurrency-limited status=509` retries and ended after ~49s. Mitigation applied live and persisted in `../k3s/plex/iptvtunerr-deployment.yaml` plus `../k3s/plex/iptvtunerr-sports-deployment.yaml`: `IPTV_TUNERR_TUNER_COUNT=1` and `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT=1`. Rebuilt/imported `localhost/iptvtunerr:cluster`, applied manifests, and both deployments rolled successfully. Runtime verification: both pods report `count=1`, `configured_tuner_limit=1`, `effective_tuner_limit=1`, and `account_pool_limit=1`; a post-rollout sports stream used `inuse=1/1` with no provider `509` in the short watch window. Remaining note: Plex `/livetv/dvrs` may still display stale `tuners="2"` until PMS refreshes device metadata, but the running Tunerr instances now enforce one locally. Longer-term gap: cross-pod provider-account leases are not shared between primary and sports pods.

**Latest (2026-04-18):** **Live TV outage after restart was a Plex DB rollback, not a Tunerr service failure:** after the restart window, `deployment/iptvtunerr` and `deployment/iptvtunerr-sports` were healthy (`/healthz` 200, primary `479` channels, sports `106`), Plex remote access was still mapped through `24.109.206.134:55555`, but Plex `/livetv/dvrs` had lost the active DVRs `757`/`760` and contained only old dead oracle/HDHR rows (`730,733,736,739,742,745,749`). Root cause: `plex-db-sync` ran at `2026-04-19T02:00Z` from the old `kspls0` primary config into standby and overwrote the newer standby Live TV DB state. Live recovery: suspended `cronjob/plex-db-sync`, re-registered primary and sports from the running Tunerr pods with `PLEX_HOST=192.168.50.148:32400` and `-register-plex=api -register-only`, creating DVR `752` (`479` activated channels) and DVR `755` (`106` activated channels), then deleted the seven dead stale DVR rows. Verification after cleanup: Plex `/livetv/dvrs` has only DVRs `752` and `755`, both devices `alive`; provider endpoints return `479` and `106` channels and populated discover hubs; plex.direct WAN `/identity` returns `200`. Durable guards applied live and in sibling ops manifests: `../k3s/plex/plex-db-sync-cronjob.yaml` now has `spec.suspend: true`, and `../k3s/plex/service.yaml` restores `service/plex -> app=plex-standby` so `https://plex.home/` returns Plex `401` instead of Traefik `404`.

**Latest (2026-04-18):** **Main-DVR channels that streamed for minutes and then died on a bad backup path now get a bounded same-upstream recovery attempt before the session is lost:** `internal/tuner/gateway_stream_upstream.go` now treats `hls_go_stalled_after_progress` as eligible for a bounded same-URL retry even when alternates exist, instead of reserving that recovery path for single-upstream rows only. This matters for real provider rows where the primary HLS path was already proven-good but the listed backup is dead (for example DNS failure), which previously converted a recoverable stall into a 4-minute `502`. Added `TestGateway_stream_hlsStallAfterProgressRetriesPrimaryBeforeDeadBackup` and kept the earlier backup-failover + single-upstream regressions green. Live cluster proof after rebuild/import/rollout: `CA| CTV COMEDY CHANNEL HD` (`/stream/176815`) hit the old `playlist refresh failed` / `ended no-new-segments` path at ~46s, then logged `upstream[1/1] hls stalled after progress; retrying same URL` and successfully spliced into a fresh playlist/segment set instead of ending the session.

**Latest (2026-04-18):** **The duplicate blank `plexKube` Live TV sources were caused by two live Plex Media Servers sharing the same server identity, not by Tunerr DVR churn or a pure client-cache bug:** after the HLS relay fixes, remote users and TV clients still saw repeated blank `plexKube` guide tabs even though canonical PMS APIs only showed two DVRs (`757`, `760`). The missing evidence was on the old host: `kspls0` still had bare-metal `plexmediaserver.service` running on both `192.168.50.85` and `192.168.50.248`, with the same machine identifier/client identifier as the k3s standby instance on `kspld0`. That made `plex.tv/api/resources` publish extra local connections and let clients rebuild a six-source Live TV graph from one server identity plus the two real DVR sources. Fix applied live: stop/disable/mask bare-metal PMS on `kspls0`, restart `plex-standby`, and verify `plex.tv/api/resources` collapses back to the intended LAN+WAN pair only (`192.168.50.148:32400`, `24.109.206.134:55555`). Immediate next step is user/client confirmation that the blank source strip collapses after refresh, not more tuner-side surgery.

**Latest (2026-04-18):** **Single-upstream-style HLS stalls are now materially more resilient on the live cluster path:** after the earlier backup-upstream failover change, the remaining disconnect class was channels whose HLS playlist stopped advancing with no alternate URL exposed to the active walk. `internal/tuner/gateway_stream_upstream.go` now gives `hls_go_stalled_after_progress` a bounded same-URL restart path only when the active stream has no remaining alternate upstreams, while multi-upstream rows still move directly to backup URLs. Added `TestGateway_stream_hlsStallAfterProgressRetriesSameUpstream` alongside the existing backup failover regression, and full `./scripts/verify` is green again. The cluster image was rebuilt/imported and `deployment/iptvtunerr` rolled successfully. Live validation improved on the previously bad CBC path: a direct in-cluster read on `/stream/176800` (`CA| CBC EAST HD`) now held for the full 70s sample window and returned `41,244,380` bytes before the client timeout, whereas the same path previously died around ~37s with `playlist refresh failed` / `ended no-new-segments`. Immediate next step if we keep pushing this lane: validate from real Plex clients whether the longer-lived relay now eliminates the user-visible disconnects, and only then decide whether further retry tuning is needed.

**Latest (2026-04-18):** **Cluster HLS mid-stream failover now works for backup-capable channels, but single-upstream provider stalls still end playback:** the gateway no longer treats `ended no-new-segments` as implicit success after bytes have started. `internal/tuner/gateway_relay.go`, `gateway_stream_response.go`, and `gateway_stream_upstream.go` now surface that condition as a retryable upstream failure so the normal `StreamURLs` walk can splice onto the next backup URL instead of dropping the client stream. Regression coverage lives in `TestGateway_stream_hlsStallAfterProgressFallsBackToBackup`. Full `./scripts/verify` is green, the cluster image was rebuilt/imported and `deployment/iptvtunerr` rolled successfully, and a live 50s in-cluster read on `CA| CTV 2 VICTORIA HD` (`/stream/177396`) stayed up for the full sample window (`37,312,360` bytes, curl timeout only). Remaining boundary from live logs: `CA| CBC EAST HD` (`/stream/176800`) still ended after ~37s because that lineup row only had one upstream and the provider returned refresh `509` / no-new-segments, so there was no alternate URL to fail over to. Immediate next step, if we keep pushing this bug class, is deciding whether single-upstream stalls should trigger controlled same-upstream re-entry instead of ending the stream.
...
**Latest (2026-04-19):** **Primary/cable DVR now has stable duplicate and sports/event filtering deployed; sports DVR was left alone:** added `IPTV_TUNERR_LINEUP_DEDUPE=stable` for strong pre-cap duplicate collapse by `tvg-id` / normalized channel name, and `IPTV_TUNERR_LINEUP_DROP_SPORTS=true` for primary/general DVRs with a separate sports tab. The primary manifest `../k3s/plex/iptvtunerr-deployment.yaml` enables both; `../k3s/plex/iptvtunerr-sports-deployment.yaml` does not. Live result after rebuild/import/rollout: primary remains `479` channels, sports remains `106`, stream/name overlap is `0`, and the sports-like classifier found `0` rows in primary. The dedupe pass dropped `8,215` strong duplicates after sports/event filtering; the sports/event filter dropped `8,180` rows before cap. The watchdog repaired the expected dead primary Plex row after rollout and settled at DVR `785` alive with `479` enabled channels; sports stayed DVR `770` alive with `106`. Follow-up bug fixed in this pass: registration/watchdog now uses the exposed/capped lineup rather than raw catalog channels, so the watchdog reports `enabled=479/479` instead of repeatedly treating `479/51284` as under-activated.

**Correction (2026-04-19):** Plex UI tab order still showed sports first because sports was the older DVR row (`770`) and primary was recreated later (`785`). Server-side lineups were correct, but Plex sorted/displayed the older sports row first. Fixed live by scaling `deployment/iptvtunerr-sports` to zero, deleting only the old sports DVR/device (`770`/`769`), then scaling sports back up so it re-registered after primary as DVR `788`. Verified final Plex order: DVR `785` `IPTV+Tunerr` first (`479`, alive, first guide IDs `1..`), DVR `788` `IPTV+Tunerr+Sports` second (`106`, alive, first guide IDs `10001..`); primary lineup still has sports-like row count `0` and stream/name overlap `0`.

**Latest (2026-04-19):** **Lineup theory pass says a dedicated general/cable NA recipe is worth testing before deployment:** live primary and sports lineups are distinct today (`479` primary, `106` sports, stream/name overlap `0`). Primary still carries roughly `36` news-like rows, `8` music-like rows, and one sports-adjacent row (`GO| SPORTSMAN CHANNEL`) because `locals_first` is a broad ranking recipe after `sports_na` exclusion, not a true general-cable recipe. A live catalog scan found `1,553` North-American/general candidates outside sports/news/music heuristics, including about `1,101` EPG-linked rows, so there is enough replacement inventory. Recommended next implementation is a new `cable_na`/`general_na` recipe that hard-excludes `sports_na`, drops/deprioritizes 24/7 music/radio, keeps only a small essential-news allowance or deprioritizes 24/7 news, and scores general entertainment/factual/movie/kids/family cable ahead of news/music before applying the `479` cap. Do not deploy this without an offline preview/test report showing `sports_na` overlap remains zero and category counts improve.

**Latest (2026-04-19):** **Provider credential leakage opportunity is fixed:** `internal/indexer/player_api.go` no longer includes full `player_api.php?username=...&password=...` URLs in `apiError.Error()` output; it now uses `safeurl.RedactURL`. Regression coverage lives in `TestPlayerAPIErrorRedactsCredentials`.

**Latest (2026-04-19):** **Cluster Plex Live TV source/DVR regression is repaired and made self-healing:** live Plex had rolled back to seven dead stale DVR rows (`730,733,736,739,742,745,749`) and no active primary/sports rows. Re-registered the stable Tunerr services, removed stale DVR/device rows, and rebuilt/redeployed `localhost/iptvtunerr:cluster` with registration hardening. Final live state: Plex `/livetv/dvrs` has exactly DVR `773` (`IPTV+Tunerr`, `479` enabled channels, alive, `http://iptvtunerr.plex.svc:5004`) and DVR `770` (`IPTV+Tunerr+Sports`, `106` enabled channels, alive, `http://iptvtunerr-sports.plex.svc:5004`). Durable fixes: cluster manifests now run `run -mode=full -register-plex=api -register-plex-interval=1m` with a host-only `PLEX_HOST=192.168.50.148:32400`, primary explicitly caps `IPTV_TUNERR_LINEUP_MAX_CHANNELS=479`, and code now honors that explicit cap in full registration mode while tolerating `PLEX_HOST` values with or without an `http(s)://` scheme. The watchdog now treats Plex `dead`/disabled device rows as stale, deletes the DVR/device pair, re-probes the HDHR endpoint, and recreates/activates the DVR instead of silently accepting a dead row. The registration log no longer prints token prefixes.

**Latest (2026-04-19):** **Investigating cluster Plex Live TV regression where clients see six empty DVR/source entries instead of exactly primary local TV plus `sports_na`:** scope is to inspect live Plex DVR/provider/resource state, identify whether the regression came from stale Plex DB rows, duplicate PMS resource advertisements, or Tunerr re-registration behavior, then make the smallest durable fix. Success criteria: Plex has only the intended two live Tunerr DVRs, primary and sports lineups remain distinct, and any recurring root cause is documented in memory bank.
...
- 2026-04-18 stream resilience follow-up: fixed the post-progress downgrade where HLS sessions that had already delivered bytes could still fall through to `all_upstreams_failed`/HTTP 502 after dead alternates. Live redeploy verified on `CA| CTV COMEDY CHANNEL HD` with a 133s / 83.3MB sustained read and final HTTP 200 after same-upstream recovery.

- 2026-04-18 release prep: packaged the Plex publication cleanup, stale-DVR reconcile, HLS same-upstream recovery, post-progress non-502 terminal handling, and shared-relay stale-attach guard for release `v0.1.44`. Live checks: main DVR healthy, sports DVR healthy, no fresh 502s in the current monitoring window.
