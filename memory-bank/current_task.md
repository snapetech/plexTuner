...
**Latest (2026-03-21):** **Harvest bridge shipped and `PAR-006` moved past preview-only:** saved Plex lineup-harvest reports now have a first-class file/runtime path via `IPTV_TUNERR_PLEX_LINEUP_HARVEST_FILE`, `/programming/harvest.json`, and preview/deck visibility, and virtual channels now publish a real synthetic-channel surface via `/virtual-channels/live.m3u` plus `/virtual-channels/stream/<id>.mp4`, not just `/virtual-channels/preview.json`. Verification is green through `./scripts/verify`. Next depth: make harvest results feed an actual Programming Manager import/apply flow, then keep deepening virtual channels into richer schedule/publishing behavior instead of only current-slot proxying.
