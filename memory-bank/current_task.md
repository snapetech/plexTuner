...
**Latest (2026-03-21):** **Harvest bridge + virtual-channel starter shipped:** saved Plex lineup-harvest reports now have a first-class file/runtime path via `IPTV_TUNERR_PLEX_LINEUP_HARVEST_FILE`, `/programming/harvest.json`, and preview/deck visibility, and `PAR-006` has restarted cleanly with file-backed virtual-channel rules plus `/virtual-channels/rules.json` and `/virtual-channels/preview.json`. Verification is green through `./scripts/verify`. Next depth: make harvest results feed an actual Programming Manager import/apply flow, and then turn virtual-channel previews into real playable/published channels instead of preview-only schedule state.
