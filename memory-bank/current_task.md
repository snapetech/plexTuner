...
**Latest (2026-03-21):** **Harvest imports are now actionable across backend and deck, and `PAR-006` is publishable:** saved Plex lineup-harvest reports now carry harvested lineup rows, `/programming/harvest-import.json` can preview/apply them as real Programming Manager recipes, the deck can now trigger those imports directly from harvested lineup cards, and virtual channels now publish a real synthetic-channel surface via `/virtual-channels/live.m3u` plus `/virtual-channels/stream/<id>.mp4`, not just `/virtual-channels/preview.json`. Verification is green through `node --check internal/webui/deck.js`, targeted tests, `bash ./scripts/ci-smoke.sh`, and `./scripts/verify`. Next depth: richer local-market heuristics for harvest imports and deeper virtual-channel schedule/publishing behavior beyond current-slot proxying.
