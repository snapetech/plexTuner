...
**Latest (2026-03-21):** **Programming Manager now curates from live evidence, not just metadata:** the deck Programming lane now includes an in-place live HLS preview for the selected curated channel, focused channel detail with 3-hour EPG and exact alternative sources, and direct virtual-schedule context alongside the existing recipe/harvest controls. The old HLS mux demo experiment was upgraded with embeddable query params and reused instead of staying a debug-only page. Verification is green through `node --check internal/webui/deck.js`, `go test ./internal/webui ./internal/tuner ./cmd/iptv-tunerr -count=1`, and `./scripts/verify`. Next depth: deeper virtual-channel publishing behavior beyond current-slot proxying, plus productizing the strongest script-only diagnostics (stream compare / channel diff / evidence bundles) into operator-facing flows.
