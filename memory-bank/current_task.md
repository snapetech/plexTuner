...
**Latest (2026-03-21):** **Diagnostics launchers + feed descriptors shipped cleanly:** the operator plane can now trigger bounded `channel-diff` and `stream-compare` captures directly (`/ops/actions/channel-diff-run`, `/ops/actions/stream-compare-run`) instead of only showing workflow guidance, Programming Manager now derives and surfaces feed descriptors from provider-presented metadata (`region | category | feedtype/fps-style tags`) across category members, curated preview cards, channel detail, and backup alternatives, and exact-backup collapse is hardened so over-normalized `tvg_id` values no longer merge distinct variants like East/West or `Plus`. While landing that slice, `./scripts/verify` exposed a real `internal/materializer` concurrency-test bug (negative `WaitGroup` counter), which is now fixed. Next depth: keep broadening parity/operator productization from script-only diagnostics toward richer in-app launch/compare flows, and keep tightening lineup/backup heuristics with real provider feedback instead of more blind normalization.
