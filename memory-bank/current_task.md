...
**Latest (2026-03-21):** **Harvest imports are now actionable and `PAR-006` is publishable:** saved Plex lineup-harvest reports now carry harvested lineup rows, `/programming/harvest-import.json` can preview/apply them as real Programming Manager recipes, and virtual channels now publish a real synthetic-channel surface via `/virtual-channels/live.m3u` plus `/virtual-channels/stream/<id>.mp4`, not just `/virtual-channels/preview.json`. Verification is green through targeted tests, `bash ./scripts/ci-smoke.sh`, and `./scripts/verify`. Next depth: expose the harvest-import action in the deck and deepen virtual channels from current-slot proxying toward richer scheduled/published channel behavior.
