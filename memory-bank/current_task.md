**Latest (2026-04-17):** **Cluster rollout plus Plex standby registration is now working:** `../k3s/plex/iptvtunerr-deployment.yaml` and `../k3s/plex/iptvtunerr-service.yaml` are live in the cluster, `plex/README.md` now documents the Tunerr build/import/secret/apply flow, and the deployment has been tightened for the real Plex use case: pinned to `kspld0`, stable identity (`IPTV Tunerr` / `iptvtunerr01`), `IPTV_TUNERR_LIVE_ONLY=true`, `IPTV_TUNERR_LINEUP_RECIPE=guide_first`, and provider pinning to the working `.env` provider entry (`line.dambora.xyz`) instead of the broken primary host. `deployment/plex-standby` was scaled up on `kspld0`, the new Tunerr device was registered into Plex as device key `751`, DVR `752` was created and repaired, and the old dead Tunerr/oracle DVRs/devices were deleted from PMS. Tunerr is definitely serving PMS traffic now (`discover.json`, `lineup_status.json`, `lineup.json`, repeated `guide.xml` fetches from `PlexMediaServer/1.43.1.10611`), and the DVR remains enabled with `463` channel mappings after the final repair. Remaining issue is narrower now: Plex's provider-layer endpoint `/tv.plex.providers.epg.xmltv:752/lineups/dvr/channels` still only surfaces `63` rows even though the DVR has `463` enabled mappings and the guide fetch is healthy, so the remaining gap appears to be inside Plex's provider presentation layer rather than the cluster deployment or Tunerr registration path.
...
**Latest (2026-04-14):** **The remaining virtual-channel transport block is now split and verified:** the slate endpoint, branded/plain virtual stream handlers, recovery relay logic, recovery-state persistence, and branding/render helpers now live in `internal/tuner/server_virtual_channel_streams.go`. That leaves `internal/tuner/server.go` focused on the tuner runtime/bootstrap, lineup shaping, diagnostics/operator shell glue, and the non-virtual remaining server helpers instead of also carrying the virtual playback transport stack. Verification is green via `go test ./internal/tuner ./internal/webui ./cmd/iptv-tunerr` and full `./scripts/verify`. Remaining larger audit themes are still backlog: deeper persona split across docs/deck/runtime, any further non-virtual `server.go` decomposition if review pressure justifies it, and any final `webui.go` cleanup if we want even smaller ownership slices.
