...
**Latest (2026-03-21):** **Harvest imports got smarter and `PAR-006` got a real schedule surface:** Programming harvest imports now report match strategies and can fall back to a local-broadcast stem heuristic when market strings differ, while virtual channels now expose `/virtual-channels/schedule.json` in addition to preview, M3U export, and current-slot playback. Verification is green through targeted tests, `bash ./scripts/ci-smoke.sh`, and `./scripts/verify`. Next depth: richer deck/operator flows around the new schedule/import diagnostics and deeper virtual-channel publishing behavior beyond a single current-slot proxy.
