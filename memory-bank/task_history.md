...
- 2026-05-07: Committed all dirty repository state, including unrelated WIP as explicitly requested, and prepared release `v0.1.55`. The batch includes Plex Live TV proxy entitlement elevation, safer local proxy defaults, dead/stale Plex DVR cleanup improvements, new runbooks/reference docs for Plex proxy frontends and VPN access patterns, and release notes in `docs/CHANGELOG.md`. Verification before commit/tag: `./scripts/verify`.
