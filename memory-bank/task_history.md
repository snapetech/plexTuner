...
- The visual probe is bounded by sample, timeout, concurrency, max feeds, max duration, and cache TTL envs. It defaults to event-only targeting when enabled so generic ABC/NBC/ESPN carriage does not all get decoded at startup.
- Added tests proving visual probing prunes a slate feed while keeping a working backup and that brief initial black does not fail a stream.
- Live benchmark against sports deployment:
  - No probe: app-side lineup delay effectively `0s`, `130` channels; rollout wait `57s`.
  - Cold fast probe old settings (`concurrency=2`, `max=2m`): app-side probe hit `2m`, kept `92/130` channels and `104/145` feed URLs.
  - Cold fast+visual old settings: app-side `2m16.8s`; fast kept `105/130`, visual event probed `3` and dropped `1`, final `104` channels.
  - Tuned fast+visual (`concurrency=4`, fast max `60s`, visual max `45s`): one run took `1m13.7s` and kept `87/113`; final manifest-aligned run took `1m42.6s`, fast kept `82/130`, visual probed `8` and dropped `4`, final `78` channels / `88` feed URLs.
- Final live state: sports image `localhost/iptvtunerr:cluster-a6bd4e9-sports-visualprobe`, `/readyz` `78`, `/discover.json` `TunerCount=88`, `/debug/active-streams.json` `0/88`.
- Verification: `go test -count=1 ./internal/indexer ./internal/tuner`, `git diff --check`, full `./scripts/verify`.

## 2026-04-21 - Add runtime sports lineup feed probing

- Added a runtime lineup probe behind `IPTV_TUNERR_LINEUP_PROBE_ENABLED` that runs after final curation/cap/guide-number offset and before `lineup.json` / `guide.xml` are exposed.
- Extended the existing smoketest probe so direct streams must return non-empty bytes and obvious provider placeholder targets such as `black.ts` / `blank.ts` fail instead of passing on HTTP `200`/`206` alone.
- Added feed-level probing that prunes bad stream URLs from a channel while preserving the channel when a backup feed works; `TunerCount=auto` and provider account auto capacity therefore derive from surviving valid feed URLs.
- Enabled the low-load probe on the sports deployment only: timeout `3s`, concurrency `2`, max duration `2m`, cache file `/var/cache/iptvtunerr/sports-lineup-probe.json`, cache TTL `10m`.
- Verification: `go test -count=1 ./internal/config`, `go test -count=1 ./internal/indexer ./internal/tuner`, `git diff --check`, full `./scripts/verify`.
- Live rollout: built/imported `localhost/iptvtunerr:cluster-a6bd4e9-sports-lineupprobe`, applied `deploy/cluster/plex/iptvtunerr-sports-deployment.yaml`, and rolled `deployment/iptvtunerr-sports`.
- Live result: `sports_na` first selected `130/51145`; lineup probe kept `91/130` channels and `101/145` feed URLs; `/readyz` reports `91` channels; `/discover.json` and `/provider/profile.json` report tuner/account capacity `101`; obvious bad patterns (`black.ts`, `SPORTSNET PPV`, `NHL TEAM`, `NFHS`, `ENDED`, `Peacock`) are absent from `lineup.json`.

## 2026-04-20 - Expand cluster `sports_na` to include NBA event feeds

- Confirmed the deployed sports lineup was not Canada-only, but was top-ordered Canadian-first and only exposed three NBA TV-style rows.
- Found current NBA event feeds in the raw 51k-channel catalog: TSN+, Peacock, and NBA Pass rows for Raptors-Cavaliers, Nuggets-Timberwolves, Knicks-Hawks, Lakers-Rockets, Celtics-76ers, and related games.
- Updated `sports_na` selection so North-American NBA/WNBA event rows with `CA (...)`, `TSN+`, or `US: NBA PASS PPV` naming are kept, while `NO EVENT STREAMING` placeholders, non-NA NBA Pass buckets, and Peacock rows stay excluded.
- Rolled cluster image `localhost/iptvtunerr:cluster-a6bd4e9` and changed the sports deployment to `IPTV_TUNERR_GUIDE_POLICY=off` so no-guide event rows are exposed to the sports-only Plex DVR; primary remains guide-healthy.
- After a user tried Peacock, confirmed `US (Peacock 031)` and `US (Peacock 017)` returned `HTTP 200 text/html` instead of HLS and failed as `invalid_hls_playlist` / Plex `502`; built and rolled sports-only image `localhost/iptvtunerr:cluster-a6bd4e9-peacockfix` to remove Peacock rows.
- Live result: sports `/lineup.json` and Plex provider `tv.plex.providers.epg.xmltv:803` both expose `126` channels, zero Peacock rows, and distinct TSN+/NBA Pass NBA event rows at guide numbers `10103` through `10122`.
- Verification: `go test -count=1 ./internal/tuner -run 'TestApplyLineupPreCapFilters_lineupRecipeSportsNA' -v`, `go test -count=1 ./internal/tuner`, `git diff --check`, `./scripts/verify` before the Peacock backout, YAML parse for `deploy/cluster/plex/iptvtunerr-sports-deployment.yaml`, `./scripts/deploy-cluster.sh`, sports rollout status, live `/readyz`, `/lineup.json`, `/guide/health.json`, sports logs, and Plex provider channel count checks.
...
- 2026-04-20: Expanded and redeployed the cluster `sports_na` Plex DVR for NBA event feeds plus capped sports carriage channels. The recipe now includes NA NBA/WNBA event rows from TSN+, Peacock, and NBA Pass PPV while rejecting `NO EVENT STREAMING` placeholders and non-NA NBA Pass rows. Peacock was fixed instead of dropped: Peacock event URLs are normalized from `.m3u8` to `.ts`, invalid/empty HLS playlists can fall back to `.ts`, Peacock MPEG-TS bypasses ffmpeg normalization, and Peacock `.ts` requests omit extra Basic auth because this provider/CDN returns zero-byte bodies when both path credentials and Basic auth are present. Added capped ABC/NBC broadcast carriage rows (12 per network, guide-linked only, news-only rows excluded) so ABC/NBC-carried games can appear in the sports DVR without flooding Plex with hundreds of affiliates. Cluster sports manifest keeps guide policy off for event rows and uses two local/account tuners with bounded retries. Verification: `go test -count=1 ./internal/tuner`, `git diff --check`, repeated Docker build/import/rollout of `deployment/iptvtunerr-sports`. Live final rollout image `localhost/iptvtunerr:cluster-a6bd4e9-sports-carriage-capped`; `/readyz` reports `165` channels with `12` ABC, `7` ESPN, `19` NBC/NBC Sports, `10` Peacock, and `7` Prime/Amazon-style rows. Live Peacock proof: `/stream/1910864` returned `14,472,616` bytes through Tunerr and provider account leases were clear afterward. Note: `/stream/1910860` was provider-empty even when fetched directly, so individual event content still depends on upstream activation.

- 2026-04-21: Fixed the Peacock spinner class where a provider-empty event row returned `HTTP 200 video/mp2t` but delivered zero body bytes after headers had already been sent to Plex. `relaySuccessfulRawUpstream` now pre-reads the first chunk for Peacock raw `.ts` bypass streams and treats zero bytes as `raw_empty_body` before committing the response. Added `TestGateway_stream_peacockTSZeroBytesFailsBeforeResponse`. Verification: `go test -count=1 ./internal/tuner`, `git diff --check`, rebuilt/imported `localhost/iptvtunerr:cluster-a6bd4e9-peacock-emptyfail`, and rolled `deployment/iptvtunerr-sports`. Live proof: `/stream/1910846` now returns `502` with `all_upstreams_failed` instead of `200/0-byte`, while `/stream/1910874` still returned `26,490,916` bytes over a 20s sample. Sports `/readyz` remains healthy with `165` channels.

- 2026-04-21: Followed up on the remaining Peacock spinner caused by Plex retrying far-future event rows even after Tunerr returned clean `502`s. `sports_na` now excludes event rows with embedded provider timestamps more than `IPTV_TUNERR_SPORTS_EVENT_FUTURE_WINDOW` ahead of current time (default `4h`), keeping active/near-live events while hiding future placeholders until they are plausibly playable. Added test coverage in `TestApplyLineupPreCapFilters_lineupRecipeSportsNA`. Verification: `go test -count=1 ./internal/tuner`, rebuilt/imported `localhost/iptvtunerr:cluster-a6bd4e9-sports-nearlive`, and rolled `deployment/iptvtunerr-sports`. Live proof: sports `/readyz` reports `158` channels, Plex API registration activated all `158`, `/stream/1910846` and `/stream/1910864` now return `404`, and remaining Peacock `/stream/1910874` returned `22,948,276` bytes over 15s.

- 2026-04-21: Tightened `sports_na` again after black-screen reports showed Plex tuning generic provider slate rows (`CA| SPORTSNET PPV` and `NHL TEAM| EDMONTON OILERS`) that returned many MPEG-TS bytes but likely no real game video. The recipe now excludes generic non-event `PPV`, `TEAM|` rows, `NFHS` PPV, and `ENDED` events while keeping NBA Pass PPV and broadcast carriage. Verification: `go test -count=1 ./internal/tuner`, rebuilt/imported `localhost/iptvtunerr:cluster-a6bd4e9-sports-slatefilter2`, and rolled `deployment/iptvtunerr-sports`. Live proof: sports `/readyz` reports `133` channels, Plex activated all `133`, and a lineup scan shows no `SPORTSNET PPV` / `NHL TEAM` / `NFHS` / `ENDED` rows while retaining `12` ABC, `19` NBC/NBC Sports, `7` ESPN, `1` Peacock, and NBA Pass PPV rows.

- 2026-04-21: Restored sports concurrency to match the multi-user goal after briefly applying a one-stream clamp in response to provider `509` overlap. `deploy/cluster/plex/iptvtunerr-sports-deployment.yaml` and the live deployment are back to `IPTV_TUNERR_TUNER_COUNT=2` and `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT=2`, while keeping the stale/slate lineup filtering. Live proof: sports `/provider/profile.json` reports `configured_tuner_limit=2`, `effective_tuner_limit=2`, and `account_pool_limit=2`; `/readyz` reports `130` channels on image `localhost/iptvtunerr:cluster-a6bd4e9-sports-staleblack`; Peacock rows are currently `0`, and known bad slate patterns are `0`.

- 2026-04-21: Replaced static sports tuner/account limits with feed-derived auto capacity. `IPTV_TUNERR_TUNER_COUNT=auto` now preserves auto through config loading and `Server.UpdateChannels` sets the advertised tuner count from unique non-empty upstream stream URLs in the exposed lineup. `IPTV_TUNERR_PROVIDER_ACCOUNT_MAX_CONCURRENT=auto` derives the account pool limit from the same lineup feed capacity while still allowing learned lower limits for specific upstream identities. Added focused tests for config auto parsing, server feed-capacity tuner count, and provider profile auto account limit. Deployed `localhost/iptvtunerr:cluster-a6bd4e9-sports-autocap` with both sports envs set to `auto`. Live proof: sports `/readyz` reports `130` channels; `/discover.json` reports `TunerCount:145`; `/provider/profile.json` reports `configured_tuner_limit=145`, `effective_tuner_limit=145`, and `account_pool_limit=145`; `/debug/active-streams.json` reports `tuner_limit=145`.

- 2026-04-21: Added and released sports lineup feed probing with optional visual black-screen probing. The sports cluster deployment now runs fast feed probes before exposing `lineup.json`/`guide.xml`, prunes bad individual stream URLs, drops channels with no valid feeds, then runs bounded ffmpeg `blackdetect` for high-risk event rows. Live validation on `deployment/iptvtunerr-sports`: final manifest-aligned cold run kept `78/130` sports rows with `88` surviving feed URLs, advertised `TunerCount=88`, and took `1m42.6s`; `/readyz` was healthy and active streams were `0/88`. Verification: `go test -count=1 ./internal/config`, `go test -count=1 ./internal/indexer ./internal/tuner`, `./scripts/verify`, and `./scripts/release-readiness.sh`. Release `v0.1.49` was pushed but GitHub release binary build failed on Windows because shared lease locking used Unix-only `syscall.Flock`; fixed with platform-specific lease-lock helpers and moved the published release to `v0.1.50`.

- 2026-04-21: Corrected the sports probe/EPG regression found after `v0.1.50` went live. Validation showed live sports exposed `99` rows with `18` placeholder-only guide rows, and Plex/tuner logs showed black-feed rows still tuning playlists with media URI `/video/black.ts`. Root cause: HLS `ProbeStream` returned success on playlist headers (`#EXTM3U` / `#EXTINF`) before scanning the media URI lines, and `IPTV_TUNERR_GUIDE_POLICY=healthy` was too broad for sports because guide-health recomputed from filtered guides and collapsed the lineup. Fixes: HLS probes scan the whole small playlist before accepting; the gateway rejects `black.ts`/`blank.ts` HLS media at tune time; sports deployment uses `IPTV_TUNERR_GUIDE_POLICY=drop_placeholders`; reapply guards against re-expanding rows from a filtered guide-health snapshot. Deployed `localhost/iptvtunerr:cluster-b29eb54-sportsfix3` to `deployment/iptvtunerr-sports`. Stable live proof after watchdog cycle: `/readyz` reports `75`, Plex watchdog reports `enabled=75/75`, guide health reports `0` placeholder-only rows and `75` real-programme rows, and old black IDs `/stream/1836256`, `/stream/1920568`, `/stream/45611` return `404`. Verification: `go test -count=1 ./internal/indexer ./internal/tuner`.

- 2026-04-21: Fixed the GitHub Actions Node 20 deprecation warning from release publishing by upgrading `softprops/action-gh-release` from `v2` to `v3` in `.github/workflows/release.yml` and `.github/workflows/tester-bundles.yml`. Upstream `v3.0.0` action metadata uses `node24`, so the next release tag validates the warning is gone. Added `v0.1.52` changelog notes. Verification: `./scripts/verify`.

- 2026-04-21: Fixed a live sports freeze on `CA| SPORTSNET WEST FHD` (`904206`). Root cause was a combination of stale shared provider-account lease files (`6h` TTL kept old completed/restarted stream leases visible) and empty same-channel shared relay sessions in the Go HLS transcode path, which made Plex retries open competing provider sessions instead of attaching to usable bytes. Reduced shared lease TTL to `2m` in code and cluster manifests, delete expired shared lease files during snapshots, and only create Go HLS shared relay sessions for non-transcode paths that can fan out bytes. Deployed `localhost/iptvtunerr:cluster-51fb168-freezefix` to primary and sports. Live validation: sports `/readyz` settled at `78`; `/provider/profile.json` showed no stale leases after the test; a 90s `Lavf/60.16.100` sample on `/stream/904206` returned `HTTP 200` with `246,397,688` bytes and no new `509`/stall logs. Verification: `go test -count=1 ./internal/tuner`; `./scripts/verify`.

- 2026-05-07: Committed all dirty repository state, including unrelated WIP as explicitly requested, and released `v0.1.55`. The batch includes Plex Live TV proxy entitlement elevation, safer local proxy defaults, dead/stale Plex DVR cleanup improvements, new runbooks/reference docs for Plex proxy frontends and VPN access patterns, and release notes in `docs/CHANGELOG.md`. Verification before commit/tag: `./scripts/verify`. Publish proof: commit `c425666` pushed to `origin/main`; tag `v0.1.55` pushed; release workflow `25514760999` passed; GitHub Release published with Linux/macOS/Windows binaries plus `SHA256SUMS.txt`.

- 2026-05-07: Hardened `plex-label-proxy -elevate-live-tv` owner-token injection while preserving the working Live TV entitlement behavior and released it as `v0.1.56`. Elevation is now limited to safe read/probe methods on explicit Live TV/provider paths plus the known transcode/playQueue helper parameters, and bait query strings on ordinary library paths no longer trigger owner-token forwarding. Docs now call out the proxy as a limited owner-token deputy rather than token disclosure protection alone. Verification: `go test -count=1 ./internal/plexlabelproxy ./cmd/iptv-tunerr`; full `./scripts/verify`; GitHub Release workflow `25516389781` passed and published binaries/checksums.

- 2026-05-07: Added manual Plex Live TV entitlement proxy documentation for operators who want to reproduce the owner-token elevation behavior without running the Tunerr proxy binary. The runbook now includes the required classification/rewrite checklist, README and docs indexes link it, the frontend reference clarifies that plain nginx/Caddy/Traefik are TLS frontends unless paired with scripting, and `docs/examples/` now contains an nginx+njs example that mirrors the hardened allowlist and rewrites both query and header token locations. Verification: `git diff --check`; `bash -n` not applicable to the njs/config examples.

- 2026-05-07: Updated the legacy Python Plex label proxy into a functional standalone fallback for the Live TV entitlement proxy pattern. `scripts/plex-media-providers-label-proxy.py` now supports `--elevate-live-tv`, owner-token fallback from env, hardened safe-method/path classification, query/header token replacement, `allowTuners` XML rewrites, provider-scoped label rewrites, and `OPTIONS`/`PATCH` proxy methods. Docs now describe the Python script as a working fallback while still preferring the Go subcommand for packaged/tested production use. Verification: `python3 -m py_compile scripts/plex-media-providers-label-proxy.py`; importlib function checks for bait-query and mutating-method blocking; `git diff --check`.
